...
func patchAgentToOIDC(ctx context.Context) {
	...
	ginkgo.By("patching mecak8s-agent to enable caller identity (discovery, no pinned JWKS)")
	patch := fmt.Sprintf(
...
// its traffic needs.
//
// The policies are REQUIRED, not decorative. kind's CNI (kindnetd) DOES enforce
// NetworkPolicy — verified: without the agent→Dex egress rule the agent could not
// fetch the JWKS and the pod CrashLoopBackOff'd, and a plain pod in the namespace
// was blocked identically while DNS still resolved. The namespace default-deny
// covers both directions for every pod.
//
// The bcrypt hash is of the literal "password" — throwaway credentials for an
// ephemeral in-cluster IdP with in-memory storage.
const dexManifests = `
apiVersion: v1
kind: ConfigMap
metadata:

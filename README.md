...
- **Core tool kit** — Read (line-numbered), Edit (read-before-edit / exact-match / uniqueness invariants), Write, Grep, Glob, **WebSearch** (Exa / Brave / SearXNG backends, degrades gracefully), and **WebFetch** (bounded public HTTP(S) text retrieval with SSRF-safe DNS pinning and HTML extraction); plus an **optional** Bash (behind a `CommandRunner` seam, so the harness runs shell-less in a locked-down pod). Plus opt-in tools: **memory** (Remember/Recall/Search plus lifecycle-aware Inspect/Forget/Undo, with a user-scoped family alongside), **skills** (Skill + a writable SkillDraft quarantine), the **Parallel** delegation tool (N isolated branches), and **ToolSearch** for progressive disclosure.
...
- **Memory** — conservative tiered memory with versioned, inspectable, reversible lifecycle; full bounded operator facts refresh in the volatile system suffix while project values remain recall-only; optional background "dream" consolidation.
- **Optional staged learning** — standard composition defaults `learning.mode` to `off`: no automatic observer is attached, while explicit reflection remains available through a lazy path. `review` reflects eligible clean completions into bounded, evidence-backed proposals without writing memory; `auto` stages first, then conservatively promotes only policy-eligible non-conflicting facts. Proposal review re-verifies ownership and evidence digests and shows a bounded redacted canonical preview before approval.

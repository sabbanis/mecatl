...
import type { V0ServerJson, V0ServerResponse } from "@/generated/types.gen";
import { mockedGetRegistryByRegistryNameV01Servers } from "@/mocks/fixtures/registry/registry_registryName_v0_1_servers/get";
...
const osvEntry = servers.find(
  (s: V0ServerResponse) =>
    s.server?.name === "com.toolhive.k8s.toolhive-system/osv",
);
const osvServer = osvEntry?.server as V0ServerJson;

const vmcpEntry = servers.find(
  (s: V0ServerResponse) =>
    s.server?.name === "com.toolhive.k8s.production/my-vmcp-server",
);

...
async function controllerIsHealthy() {
  try {
    const response = await fetch("http://127.0.0.1:8788/status", { signal: AbortSignal.timeout(1_500) });
    return response.ok;
  } catch {
    return false;
  }
...
async function ensureController() {
  ...
  controller.once("exit", () => {
    controller = null;
    if (!stopping) controllerRestart = delayRestart(() => { controllerRestart = null; void ensureController(); });
  });
}
...
function startWeb() {
  ...
  web.once("exit", () => {
    web = null;
    if (!stopping) webRestart = delayRestart(() => { webRestart = null; startWeb(); });
  });
}
...
startWeb();
const healthCheck = setInterval(() => { void ensureController(); }, 3_000);

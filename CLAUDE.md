...
- **Per-session/selector engines mount the server-global MCP tools** by reusing `Build`'s already-connected shared manager (`globalMgr.Tools()`, threaded into `sessionEngineFactory`) — a selector session (any `/models` pick) must NOT get a core-only catalog or it silently strips github/slack/fetch/etc. The shared `globalMgr` is owned by `Build`: reused-not-reconnected, and its `Close` is NEVER folded into the per-session `SessionEngineResult.Close` (a per-session `CloseSession` closing it kills MCP for all other sessions). Order core → global MCP → client MCP → Task/Team (global-wins on collision); `globalMgr` is also the `reference:`-resolution mainMgr for per-session subagents.

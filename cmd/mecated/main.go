...
type config struct {
	...
	shell                string
	noBash               bool
	detachedRuns         bool          // detached runs (ADR 0321, --detached-runs, default OFF)
...
func applyPostureCLI(cfg config, diag port.Diagnostics) error {
	...
	case app.PostureYolo:
		...
		if cfg.detachedRuns {
			// ADR 0321 decision 6: a yolo run implicitly assumes a human is
			...
			slog.Warn("detached runs REFUSED under posture yolo: --detached-runs is enabled but a detached run would remove the last human checkpoint the yolo contract assumes. Either drop --detached-runs (attached runs only) or run a lower posture (strict/trusted/auto). See ADR 0321 decision 6.")
		}
	}
	return nil
}

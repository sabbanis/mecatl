...
func runWithOptions(argv []string, options runOptions) error {
	...
	// Manual two-signal handler (ADR 0322 Scenario 4): the shared DetachSignalState
	...
	// The detached-runs capability seam (ADR 0322 Scenario 4): reads the REAL
	...
	// The control-only Converse cancel (ADR 0322 Scenario 4): the second-Ctrl+C
	...
	// running (ADR 0322 Scenario 4 / AC4.2). wireDetachCancel's fresh bounded
...
// detachable is the FIRST-signal detach decision (ADR 0322 Scenario 4): can the
...
// session (ADR 0322 Scenario 4): Cancel{SessionId} on a FRESH stream, then
...
// (ADR 0322 Scenario 4). The closure MUST run on a FRESH, bounded context per

...
func TestRelayoutSyncStuckAfterTransientShrink(t *testing.T) {
	...
	if !m.stuck || !m.vp.AtBottom() {
		t.Fatalf("precondition: view should start stuck at the bottom (stuck=%v atBottom=%v)", m.stuck, m.vp.AtBottom())
	}
...
func TestRelayoutNoOpWhenHeightUnchanged(t *testing.T) {
	...
	if m.stuck != beforeStuck {
		t.Errorf("no-op relayout changed stuck %v → %v (stray syncStuck)", beforeStuck, m.stuck)
	}
}

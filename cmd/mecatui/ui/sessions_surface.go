...
// detached run (ADR 0322 Scenario 3) via WatchSessionEvents, rather than adopting
...
func (s *sessionsState) handleActionKey(msg tea.KeyPressMsg) (tea.Cmd, bool) {
	...
	if key.Matches(msg, s.deps.keys.Choose) {
		...
		// A RUNNING row is a reattach candidate (ADR 0322 Scenario 3): the server
...
func stateBadge(state string) string {
	...
	case "running":
		...
		// candidate, ADR 0322 Scenario 3) from the play-triangle cursor marker

...
// asserts it carries ONLY the ambient base fields (theme/keys/marks/caps) and
// NONE of the archived deps-per-call wideners (width/ctx/lifecycle/nextEpoch/
...
// docs/design/surface-migration-plan.md §4 decision 8). This is the anti-
// regression guard for the deps-on-state redesign: a deps-per-call widening
// (the archived past design) fails here the moment a non-ambient field lands.
func TestSurfaceDepsIsAmbientOnly(t *testing.T) {
	st := reflect.TypeOf(surfaceDeps{})
	want := []string{"theme", "keys", "marks", "caps"}
	if st.NumField() != len(want) {
		t.Errorf("surfaceDeps has %d fields, want exactly %d (theme/keys/marks/caps only)", st.NumField(), len(want))
	}
	banned := []string{"width", "ctx", "lifecycle", "nextEpoch", "focusInput"}
	for i := 0; i < st.NumField(); i++ {
		f := st.Field(i)

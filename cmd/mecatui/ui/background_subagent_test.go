...
func TestSubagentFocusBackgroundNote(t *testing.T) {
	...
	out := stripANSIstr(renderSubagentFocus(th, running, "subagent-p1", 0))
	...
	out = stripANSIstr(renderSubagentFocus(th, done, "subagent-p1", 0))
	...
	fg := []subagentLane{{childID: "subagent-p2", goal: "quick trace"}}
	out = stripANSIstr(renderSubagentFocus(th, fg, "subagent-p2", 0))

...
type config struct {
	...
	// --subagent-model-router flag is a tri-state KILL-SWITCH: subagentModelRouter holds
	// its value and subagentModelRouterSet records whether it was given. =false sets
	// app.Config.RouterDisabled (forces the router OFF despite a taxonomy); a bare flag /
	// =true is the now-redundant enable and emits a one-time deprecation WARN; unset
	// leaves routing governed by taxonomy presence. UNLIKE the ask-reviewer, the router
	// is MEANINGFUL under mecatui — it picks the child's model before it runs, in both
...
func parseFlags(args []string) (config, error) {
	...
	fs.Visit(func(f *flag.Flag) {
		...
		case "subagent-model-router":
			...
			// embeddedConfig can distinguish unset (router governed by the taxonomy) from

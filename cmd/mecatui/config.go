...
type config struct {
	...
	// pointer exists for the connect target (ADR 0322 Scenario 3): the operator
...
func parseTransportFlags(mode transportMode, out io.Writer, args []string, browseSessions ...bool) (*flag.FlagSet, config, error) {
	...
	fs.BoolVar(&cfg.forceNew, "new", false, "start a FRESH session even when a persisted last-session pointer exists for this connect target (ADR 0322). The no-flag default auto-branches on the persisted pointer: running → reattach via WatchSessionEvents; idle/terminal → resume as today; none → fresh. --new skips the pointer and always starts fresh (mutually exclusive with --resume/--resume-latest)")

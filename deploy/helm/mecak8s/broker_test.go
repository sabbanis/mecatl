...
func TestUnifiedChartReleaseStagesFreshWorkspace(t *testing.T) {
	...
	for _, step := range job.Steps {
		...
		// Execute the real staging commands without the Linux-only sed rewrite.
		staging, _, ok := strings.Cut(step.Run, "sed -i")
		if !ok || !strings.Contains(staging, "cp -R") {
			t.Fatal("package step has no chart staging before digest rewrite")
		}
		cmd := exec.Command("sh", "-eu", "-c", staging)
		cmd.Dir = t.TempDir()
		digest := "sha256:" + strings.Repeat("a", 64)
		cmd.Env = append(os.Environ(), "CHART_PATH="+chart,
			"AGENT_IMAGE_REF=ghcr.io/stacklok/mecatl/mecak8s@"+digest,
			"BROKER_IMAGE_REF=ghcr.io/stacklok/mecatl/mecabroker@"+digest,
			"AGENT_IMAGE_DIGEST="+digest, "BROKER_IMAGE_DIGEST="+digest)
		if output, err := cmd.CombinedOutput(); err != nil {
			t.Fatalf("stage release chart in fresh workspace: %v\n%s", err, output)
		}
		if _, err := os.Stat(filepath.Join(cmd.Dir, "dist", "mecak8s", "Chart.yaml")); err != nil {
			t.Fatal(err)
		}
		return
	}

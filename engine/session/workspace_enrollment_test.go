...
func newEnrollmentSession(t *testing.T) *Session {
	t.Helper()
	s := New("enrollment-session", ModeDefault, ".", Limits{}, time.Unix(1, 0).UTC())
...
func TestBeginWorkspaceEnrollmentPrerequisites(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			s := New("s", ModeDefault, ".", Limits{}, time.Unix(1, 0).UTC())
			tc.setup(s)
			state := s.State
...
func sessionWithEnrollmentAuthority(t *testing.T) (*Session, PendingWorkspaceEnrollment) {
	t.Helper()
	s := New("enrollment-complete", ModeDefault, ".", Limits{}, time.Unix(1, 0).UTC())
...
func TestWorkspaceEnrollmentToolNamesPreserveLegacyGrammar(t *testing.T) {
	...
	s := New("legacy-authority", ModeDefault, ".", Limits{}, time.Unix(1, 0).UTC())

...
- **Captured Project Session binding foundation** (issue #620, [ADR 0234](../docs/adr/0234-project-working-and-reference-folders.md)) — `session.ProjectBinding`, `ProjectSourceBinding`, and the locator-free `ProjectProvenance` carry inert creation-time Project association data. Snapshot round trips and `eventsource.SessionMeta` preserve it while legacy and ordinary Sessions remain unbound. New types and methods are Added (minor); existing struct fields are classified Changed below.
...
- **Captured Project binding fields on existing Session metadata** (issue #620, [ADR 0234](../docs/adr/0234-project-working-and-reference-folders.md)) — `session.Session.Project` persists the inert captured binding and `port.SessionDiscoveryMeta.Project` exposes only its locator-free compact provenance. Legacy and ordinary sessions retain zero values, but the struct additions break external unkeyed literals and are therefore Changed for a pre-v1 minor bump.

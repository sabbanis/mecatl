...
- **Child workspace-view options** (repair-wave task 05) — removed `agent.WithSharedChildWorkspace`, `agent.WithTeamSharedBaseWorkspace`, and `agent.WithTeamToolSharedBaseWorkspace`. Their replacements are the read-ledger factory options listed above: base-sharing/direct-write children now preserve the exact parent `Workspace` while receiving a fresh ledger, and forked children preserve the forker's exact content backend while replacing its ledger. Changed/breaking (pre-v1 a minor bump).

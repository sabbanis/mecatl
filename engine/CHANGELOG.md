...
- **`tool.ReadLedger`, `tool.ErrLedgerUnavailable`** ([ADR 0278](../docs/adr/0278-persistent-read-before-write-ledgers.md)) — the storage-independent, context-aware read-before-write evidence capability a `Workspace` composes with, selectable independently of its file-content backend. `RecordRead(ctx, key, version) error` and `RecordedVersion(ctx, key) (version, ok, err)` distinguish a valid recorded token, ordinary absence, and an UNAVAILABLE/corrupt lookup (`err != nil`) as three separate outcomes — the third is new: the prior in-memory-only ledger could never fail, so callers had no failure mode to classify. `engine/adapter/memledger` is the in-memory reference implementation (a fresh instance is the default a `Workspace` selects when no durable ledger is configured); `engine/adapter/ledgerconformance` is the shared behavioral suite every implementation runs. Both additions are **Added = minor**.

### Changed

- **`tool.Workspace.RecordRead`/`RecordedVersion`** ([ADR 0278](../docs/adr/0278-persistent-read-before-write-ledgers.md)) — change from `RecordRead(path string, version FileVersion)` / `RecordedVersion(path string) (version FileVersion, ok bool)` to `RecordRead(ctx context.Context, path string, version FileVersion) error` / `RecordedVersion(ctx context.Context, path string) (version FileVersion, ok bool, err error)`. The read-ledger is now storage-independent and context-aware: a `Workspace` composes with a selected `tool.ReadLedger` (a fresh in-memory one by default) instead of an inline map, so a genuine ledger storage failure — previously impossible to express — is now a THIRD outcome distinct from ordinary absence, and callers (the built-in Edit and existing-file Write tools) must fail closed on it. Changed/breaking (pre-v1 a minor bump); every in-tree `tool.Workspace` implementation (osfs, memfs, nofs, the ACP fs-delegation workspace, remoteenv, and the various engine/agent stub workspaces) is migrated in this same change.

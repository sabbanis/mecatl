...
func testEnvironment(ws tool.Workspace, runner tool.CommandRunner) tool.Environment {
	return tool.MustEnvironment(session.EnvironmentRef{Kind: session.EnvKindMem, ID: "test"}, ws, testReadLedger(), runner)
}
...
func memEnv(root string) tool.Environment {
	return tool.MustEnvironment(session.EnvironmentRef{Kind: session.EnvKindMem, ID: root}, memfs.NewWorkspace(root), testReadLedger(), nil)
}
...
func forkEnv(ws tool.Workspace) tool.Environment {
	return tool.MustEnvironment(session.EnvironmentRef{Kind: session.EnvKindMem, ID: ws.Root()}, ws, testReadLedger(), nil)
}
...
func memEnvRunner(root string, runner tool.CommandRunner) tool.Environment {
	return tool.MustEnvironment(session.EnvironmentRef{Kind: session.EnvKindMem, ID: root}, memfs.NewWorkspace(root), testReadLedger(), runner)
}

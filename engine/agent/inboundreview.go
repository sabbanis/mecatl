...
func (r *Run) publishInboundDetail(ctx context.Context, sessID session.SessionID, assessment inboundAssessment) {
	...
	if assessment.err != nil {
		r.publishReviewDetail(ctx, ReviewDetail{SessionID: sessID, ReviewID: assessment.request.ReviewID, Concern: "Inspection could not complete; no unsafe finding was inferred.", NextAction: "Release this already-produced result once or cancel. The tool is not run again."})
		return
	}

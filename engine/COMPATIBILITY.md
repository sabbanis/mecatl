...
| creation metadata: id, mode, limits, workspace, profile, provider/model selector, reasoning-effort, debug selected-global-MCP names and exact direct-tool ceiling, broker enrollment provenance and wrapper inventory, **session kind and relationship**, adoption source/request digest, createdAt | **MUST** (supplied out-of-band) | **NOT in any event** — provided by the caller via `eventsource.SessionMeta`; an absent legacy kind restores as fail-closed `unknown` |
...
workspace, profile, provider/model selector, reasoning-effort, debug selected-global-MCP names and exact tool ceiling, broker enrollment provenance and wrapper inventory, authoritative title/provenance,

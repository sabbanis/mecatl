...
| creation metadata: id, mode, limits, workspace, profile, provider/model selector, reasoning-effort, **session kind and relationship**, captured Project binding, adoption source/request digest, createdAt | **MUST** (supplied out-of-band) | **NOT in any event** — provided by the caller via `eventsource.SessionMeta`; an absent legacy kind restores as fail-closed `unknown` |
...
session kind/relationship, captured Project binding, adoption source/request digest, or createdAt. The caller — who created or discovered the session —

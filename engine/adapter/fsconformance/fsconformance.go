...
func Run(t *testing.T, newWS func(t *testing.T) tool.Workspace) {
	...
	t.Run("read ledger records and looks up version", func(t *testing.T) {
		...
		if _, ok, err := ws.RecordedVersion(ctx, "led.txt"); err != nil {
			t.Fatalf("RecordedVersion before record: %v", err)
		} else if ok {
		...
		if err := ws.RecordRead(ctx, "led.txt", ver); err != nil {
			t.Fatalf("RecordRead: %v", err)
		}
		if got, ok, err := ws.RecordedVersion(ctx, "led.txt"); err != nil {
			t.Fatalf("RecordedVersion after record: %v", err)
		} else if !ok || !got.Equal(ver) {
		...
		if got, ok, err := ws.RecordedVersion(ctx, "led.txt"); err != nil {
			t.Fatalf("RecordedVersion after behind-the-back write: %v", err)
		} else if !ok || !got.Equal(ver) {

 ...
 * repository's bin/mecated (`task build`). The suite skips, loudly, when it is
 * absent so the offline gates never depend on a Go toolchain.
 */
...
describe.skipIf(!available)("Studio BFF against a spawned mecated --mock", () => {
  ...
  it("bounds a durable replay and resumes from its cursor without duplication", async () => {
    ...
    // Exact resume: nothing already delivered comes back.
    ...
    for (let hop = 0; hop < 20; hop += 1) {
      ...
      if (end?.type !== "run.truncated") break;
    ...
    if (bad.status === 400) {
      ...
      // A daemon that accepts an unknown cursor must still not replay the whole
      // history unbounded: the bound applies either way.
      const events = frames(await bad.text()).filter((frame) => frame.type === "run.event");
      expect(events.length).toBeLessThanOrEqual(2);
    }
  });
});

...
/**
 * AC3.12: state-changing `/api/v1` requests must be same-origin (Fetch
 * Metadata, or an `Origin` equal to the configured origin) AND carry the
 * double-submit token. SameSite=Lax on the cookies is the first layer; this
 * is the second.
 */
export function sameOriginMutations(options: SecurityOptions): MiddlewareHandler<AppEnv> {
  return async (context, next) => {
    ...
    const sameOrigin =
      ...
      (fetchSite === undefined && origin === expected) ||
      (fetchSite !== undefined && origin === expected);
    if (!sameOrigin) {
      return problem(
        context,

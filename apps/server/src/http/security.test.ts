...
describe("security middleware", () => {
  ...
  it("auth routes are rate limited per client with 429 and Retry-After", async () => {
    ...
    const hit = (address: string) => app.request("/api/v1/auth/session", {}, peer(address));

    for (let index = 0; index < 3; index += 1) expect((await hit("10.0.0.1")).status).toBe(200);
    ...
    // Non-auth routes are never limited.
    for (let index = 0; index < 10; index += 1) {
      expect((await app.request("/api/health", {}, peer("10.0.0.1"))).status).toBe(200);
    }
  });
  ...
  it("the client address honours STUDIO_TRUSTED_PROXY_HOPS", async () => {
    ...
    expect(
      (await zero.request("/api/v1/auth/session", { headers: forwarded }, peer("10.0.0.1"))).status,
    ).toBe(200);
    expect(
      (
        await zero.request(
    ...
    const one = limited(1);
    expect(
      (await one.request("/api/v1/auth/session", { headers: forwarded }, peer("10.0.0.1"))).status,
    ).toBe(200);
    expect(
      (
        await one.request(
    ...
    const two = limited(2);
    expect(
      (await two.request("/api/v1/auth/session", { headers: forwarded }, peer("10.0.0.1"))).status,
    ).toBe(200);
    expect(
      (await two.request("/api/v1/auth/session", { headers: forwarded }, peer("10.0.0.2"))).status,
    ).toBe(429);
    expect(
      (
        await two.request(

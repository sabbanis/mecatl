...
export function runtimeConfigFromEnvironment(
  ...
  if (baseUrl !== undefined && baseUrl !== "") {
    const url = parseHttpUrl("MECATL_BASE_URL", baseUrl);
    if (mockValue === "1") {
    ...
    const resource =
      resourceUrl === undefined || resourceUrl === ""
        ...
        : canonicalResource(parseHttpUrl("MECATL_RESOURCE_URL", resourceUrl));
    return {
...
function parseHttpUrl(variable: string, raw: string): URL {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {

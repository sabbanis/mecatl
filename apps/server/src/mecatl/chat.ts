    ...
    async *activity(sessionId, request) {
      ...
      try {
        for await (const envelope of activity) {
          ...
          // Only REPLAY is bounded. A live turn streams in full.
          if (envelope.phase === "replay") {
            if (replayed >= replayMax) {
              yield { delivery: { cursor: lastCursor, reason: "bound", type: "run.truncated" } };
              return;
            }
            replayed += 1;
          }
...
export function runError(error: unknown): Extract<RunStreamEvent, { type: "run.error" }> {
  return {
    ...
    message: error instanceof Error ? error.message : "Mecatl could not complete the run.",
    type: "run.error",
  };
}

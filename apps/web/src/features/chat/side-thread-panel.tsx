...
import { ModelEffortMenu } from "./model-effort-menu";
...
function useSideThreadRun(sessionId: string) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        await consume(response.stream, crypto.randomUUID(), "", true);
  ...
  async function consume(
    ...
    return finalRunFailure(state, prompt);
  }
  ...
  async function sendPrompt(prompt: string) {
    ...
    try {
      ...
      const failure = await consume(response.stream, assistantId, prompt, false, seeded);
      if (streamFailure && !controller.signal.aborted) throw streamFailure;
...
function groupModels(models: ComposerModelOption[]): Map<string, ComposerModelOption[]> {

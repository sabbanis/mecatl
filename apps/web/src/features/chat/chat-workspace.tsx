...
export function ChatWorkspace({ sessionId }: { sessionId?: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const failure = await consumeRun(response.stream, crypto.randomUUID(), "", true);
        if (streamFailure && !controller.signal.aborted) throw streamFailure;
        if (failure) setFailedRun(failure);
  ...
  async function sendPrompt(
    ...
    try {
      ...
      const failure = await consumeRun(response.stream, assistantId, prompt);

      if (streamFailure && !controller.signal.aborted) {
        throw streamFailure;
      }
      if (failure) {
      ...
      setLiveUsage(undefined);
      setIsRunning(false);
      if (activeSessionId && !controller.signal.aborted) drainNextQueuedMessage(activeSessionId);
    }
  }
  ...
  async function handleComposerSend(
    ...
    if (action === "steer" && runId) {
      try {
        await steerRun({ body: { text: prompt }, path: { runId, sessionId }, throwOnError: true });
  ...
  async function retryFailedRun() {
    ...
    try {
      ...
      const failure = await consumeRun(response.stream, assistantId, failedRun.prompt);
      if (streamFailure && !controller.signal.aborted) throw streamFailure;
      if (failure) {
        setFailedRun(failure);
        saveFailedRun(sessionId, failure);
      } else {
      ...
      setLiveUsage(undefined);
      setIsRunning(false);
      if (!controller.signal.aborted) drainNextQueuedMessage(sessionId);
    }
  }
  ...
  async function consumeRun(
    ...
    for await (const delivery of stream) {
      ...
      if (delivery.type === "run.truncated") {
        // The replay was bounded (or hit a durable hole), so this stream is not
        // the whole story: the transcript route owns the authoritative history.
        setNotice(
          delivery.reason === "gap"
            ? "Some earlier activity is missing from the live log. The transcript below is authoritative."
            : "Showing the most recent activity. Earlier history is in the transcript.",
        );
        if (sessionId) {

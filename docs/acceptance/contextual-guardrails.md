...
Add `ToolReviewer ToolReviewer`, `ReviewDetails ReviewDetailSink`, `ReviewTaskWindow int`, and `PlanApprovals PlanApprovalStore` to `agent.Deps`. `ReviewTaskWindow` is clamped to 1–3 with zero/default 1 and controls the last K authenticated root prompts. `session.Message.UserPromptProvenance` is a fail-closed tri-state persisted by snapshots and the log-only user-prompt event: only the narrow principal-recording aggregate method used by accepted root prompt and steer ingress creates positive provenance; ordinary/legacy user-role records, model output, and harness continuations cannot be upgraded from content. `PlanApprovalStore` is an optional narrow process-local seam over `PlanApprovalReceipt{Ref, SessionID, Call, TargetMode}`; the plan-specific type makes duplicate origin/kind fields unnecessary. It records only a real positive plan callback after the target-mode transition succeeds and atomically consumes (including discarding malformed or mode-mismatched values) after the next accepted execution prompt. Session-close/clear cleanup remains a concrete composition lifecycle operation rather than an engine-store interface method. It is distinct from permission learning, guardrail repeat grants, result release, snapshots, and event replay. These are engine API additions and require the API snapshot and changelog update during implementation. Do not widen `port.LLMRequest`, `port.HookRunner`, or `port.PermissionPolicy`. Composition builds the reviewer through the normal child-engine factory with the checker provider/model pair, context window, prompt config, cancellation, and token accounting re-derived for that pair.

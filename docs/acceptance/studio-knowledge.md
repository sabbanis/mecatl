...
**Work classification:** Bounded — adds the knowledge product surface (configured and learned skills, learning proposals, session reflection, user memory and its consolidation) to the Studio BFF and the knowledge workspace to the web app, inside the boundary ADR 0350 fixed; no new durable architecture decision.
**Decision record:** None — the boundary and the published-SDK rule are ADR 0350's; the daemon-side learning, skills, and memory contracts already exist (see [architecture](../architecture.md#evidence-backed-reflection)); this plan only projects them through the BFF.
...
views, a learned-skill detail page with a unified diff, the learning review, and the memory
...
([architecture](../architecture.md#evidence-backed-reflection)); Studio reads each flag live and degrades per surface rather than failing wholesale ([ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).
...
forwards the daemon's concurrency tokens unchanged ([ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).

**Acceptance:**
- AC2.1: detail returns the requested version's body and metadata with the daemon's per-skill action flags; diff returns the unified diff between two versions; the changes route lists lifecycle history newest first with `complete`.
...
- AC2.2: `activate`, `archive`, and `reject` call the SDK operation of the same name with `{id, ownerAgent, expectedRevision}`; `rollback` calls `rollback` with `targetVersion` defaulting to the request's `version`; a missing token is a `400` before any SDK call.
...
- AC2.3: the daemon's conflict on a stale `expectedRevision` is relayed as problem details with the daemon's code and status, never retried.
...
applies or dismisses ([architecture](../architecture.md#evidence-backed-reflection); [ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).
...
Navigation and routing follow the chat and schedules layers ([studio-chat](studio-chat.md); [ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).
...
- AC5.2: the learned-skill detail classifies unified-diff lines without treating file headers as changes.

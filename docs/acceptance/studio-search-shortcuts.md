...
**Decision record:** None — the boundary is ADR 0350's; these surfaces add no new ingress, they read what earlier layers already authorised.
...
could not already see ([ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md), decision 3).
...
- AC2.2: keycaps render platform modifiers and named keys; matching accepts primary modifiers, plain keys, aliases, and punctuation, rejects missing, extra, or incomplete modifiers, and allows only primary-modifier commands and Escape to fire inside inputs.
...
the owning components use; the palette mounts in the top nav ([studio-chat](studio-chat.md); [ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).

**Acceptance:**
- AC3.1: the help-feature rows read every capability as disabled when the daemon reports nothing on, flip on with the capability, and read memory consolidation off the exact gate the Consolidate button uses.

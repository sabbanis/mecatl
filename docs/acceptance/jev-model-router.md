...
**Status:** proposed, 2026-09-22 - Plan / Interface PR #1735 and stacked implementation PR #1738 remain open. All decisions below, including the default-category correction, capability-oriented prompt, shared decision evidence, live UI and session-debugger projection, and testing/debugging guidance, are directly approved in conversation. Direct approval is not a claim that the plan PR merged. The earlier explicit waiver of the merged-plan prerequisite continues to apply to the stacked implementation.
...
- **Events / persistence:** Candidate data is validated and model aliases are resolved locally before projection, even when confidence rejects the candidate. A mapping failure retains `CandidateCategory` and leaves `CandidateModel` empty. A child-engine factory rejection retains both candidates, records `Outcome: "fallback"`, keeps the existing `routing_reason: "route-target-unavailable"`, and leaves `model` as the inherited model that actually ran. On accepted routes, only the existing `routed_category` and `routed_model` assert the route, while `model` remains the actual model. Do not duplicate those final facts in `RoutingDecision`.
  ...
  Persist the nested value through the existing event relay and stores. Team roster evidence remains on `team.start`; repair the currently unreachable debugger projection only by joining that roster to a unique same-owner retained lineage record keyed by `(team_id, member_name)`, failing closed on zero or ambiguous matches. No fourth delegation lifecycle, backend wire field outside `RoutingDecision`, or new storage backend is introduced.
...
The evidence follows the three existing lifecycle families from [ADR 0083](../adr/0083-routing-reason-on-delegation-start.md), the [live model-routing architecture](../architecture/providers.md#the-semantic-model-router-phase-5), and the debugger boundary in [the TUI guide](../tui.md#debug-a-stored-session).

**Acceptance:**
- AC6.1: the exact protobuf fields and optional-presence semantics round-trip through generated Go/TypeScript and handwritten SDK events. Real persisted Subagent, Parallel, and Team start events replay the same decision through `InspectSession delegation`; historical absence stays absent. Team roster recovery uses only the unique same-owner `(team_id, member_name)` retained-lineage join and fails closed on zero, ambiguity, stale lineage, or cross-owner data. Tests prove no child-content or hostile/secret metadata leak. An unavailable event log uses the debugger's existing unavailable/error behavior; a missing or failed event append leaves routing evidence absent and never reconstructed by inference or falsely labeled as a skipped classifier. Append failures retain the existing warn-and-continue run behavior, with offline failure-injection coverage.

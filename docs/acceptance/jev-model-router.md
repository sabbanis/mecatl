...
**Status:** proposed, 2026-09-21 — Plan / Interface PR #1735 is in review. All decisions below, including the outcome-taxonomy correction, are directly approved in conversation; they are not approved by a merged plan. The user explicitly waived the merged-plan prerequisite for the stacked implementation.
...
- **Compatibility / migration:** Existing LLM configuration remains on the `llm` backend by default and behaves byte-for-byte except that an observed LLM classifier deadline now reports `timeout` rather than the former conflated `cancelled`; consumers of router miss reasons must accept that documented public reason refinement. The existing `default-category` keeps its LLM advisory behavior and becomes available to unshipped Jev configuration as the same advisory hint, removing only the unjustified Jev validation restriction; it is never a runtime fallback. Jev is opt-in through `backend: jev`; inactive routing creates no client or network capability. The proposed `jev-*` reason strings have not shipped and are replaced by the common taxonomy, so they require no wire migration. Pin the client dependency `github.com/stacklok/typesafe-go` to SDK version v0.1.0, independently from pinning the service request's Jev model default to `jev-1.13.0`; neither pin follows a moving alias. Implementation must update `user-docs/features/choose-models.md`, the generated configuration reference source, and applicable deployment credential pages for `mecated`, `mecatequi`, `mecak8s`, and embedded `mecatui`; generated Markdown is not hand-edited.

## In scope — 4 scenarios, in implementation order

The 15 existing named proof names remain unchanged; AC2.5 adds `TestADR_0350_Scenario2_DefaultCategoryHint` as the sixteenth.
...
- AC2.5: a real-composition LLM router and the actual SDK-backed Jev adapter both accept the current user-shaped medium `default-category` configuration. Jev renders the same optional advisory hint in trusted instructions without changing its offered-set authority; the rendered hint, including quote-escaping expansion, participates in the 64 KiB bound. The hint does not change confidence, protocol-error, or inherited-model fallback behavior.
...
- AC4.2: before SDK marshalling, the adapter measures the UTF-8 bytes of the fully rendered Jev instructions and request data: fixed instruction text plus the optional `default-category` hint after `%q` quote-escaping expansion, the task, selected model, fixed question identifier, and every category name and description. A sum over 64 KiB or more than 255 categories returns `input-over-limit` without I/O; the adapter never truncates the task or taxonomy. The SDK request contains only these controlled strings/maps and uses no user-defined `MarshalJSON`. The 64 KiB bound is a proposed local text-input guard, not an authoritative tokenizer or guarantee that every service-token/context overflow is detected; server-side size/context rejection remains a structured SDK/API outcome or generic `classifier-error` fallback. The 255-category cap follows current reviewed service guidance. Fixed text and the optional hint keep payload shape predictable.

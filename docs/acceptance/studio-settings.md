...
**Work classification:** Bounded — adds the read-only runtime-settings and storage-health surface to the Studio BFF and the settings workspace to the web app, inside the boundary ADR 0350 fixed; no new durable architecture decision.
**Decision record:** None — the boundary and the published-SDK rule are ADR 0350's; provider and routing configuration stay deployment-managed (the operator-tier posture of [architecture](../architecture.md), "Build identity" and providers), so Studio only displays them.
...
learning, models, storage, and about sections — and the chat workspace's model picker, stubbed
since the chat layer, becomes live over the new inventory.
...
- [x] Chat's model picker. — Decision: the chat and side-thread composers re-wire to `getRuntimeSettings` and the browser's disabled-model preference, restoring the prototype behaviour the chat layer stubbed with an empty model list.
...
The daemon's storage health is an operator capability; the BFF maps it without inventing figures ([ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).
...
Browser-side summaries are pure functions with ported tests; components render from them ([ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md), decision 3).
...
- AC3.3: the models section lists providers and models from the runtime-settings inventory, lets the user disable models locally (`studio.chat.models.disabled`), and renders the management controls disabled with the daemon's reasons; the learning and memory sections reuse the knowledge layer's review and consolidation components; the profile section hosts the sign-out control.
...
Navigation follows the earlier layers ([studio-chat](studio-chat.md); [ADR 0350](../adr/0350-mecatl-studio-in-repo-web-ui.md)).
...
- AC4.2: the chat and side-thread composers query `getRuntimeSettings` and offer the inventory minus locally disabled models, and image attachments follow the draft model's `image` flag again.
  - verify: inspection — `chat-workspace.tsx` and `side-thread-panel.tsx` diffs against the chat layer's stub.

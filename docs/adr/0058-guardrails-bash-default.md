# ADR 0058 — Add Bash to the default guardrail rule set with a read-only pre-filter
...
- [ADR 0059](./0059-guardrails-human-override.md) — the one-shot `/guardrail-allow` human override that recovers a residual false-positive the Bash rubric's strict posture still leaves.

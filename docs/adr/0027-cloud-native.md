...
**Session-scoped vMCP broker re-audit (ADR 0245, Stage 3 command-root proof).** List 1
rows 64–67 record the optional root-internal Runtime, Service-owned session-local
transport wrappers, parked-completion health counter, durable pending authorization
correlation, and per-session expiry timer ownership. Row 27 remains the live lease
ownership and renewer: a second process fails closed while its holder is live, with no
routing or continuation handoff. List 2 row 38 records the opaque enrollment identity
and pending authorization snapshot. A restart rebuilds Runtime, transports, grants,
callback transactions, and expiry workers from scratch; it never reattaches a
prior-process grant or timer. Command roots mount the existing HandlerBundle only on
their primary listener, adding no listener or route-table resource.
...
| 41 | Session-scoped vMCP broker routes, session/backend bindings, pending callback transactions, downstream grants, disconnect marks, refresh operations, and per-session streaming-HTTP clients (ADR 0245; List 1 rows 64–65) | root-internal `internal/adapter/vmcpbroker.Runtime`; currently only hermetic in-process construction, with no `app.Build`/server/mecatui wiring | process restart drops all runtime and session state. Static routes can be re-derived from strict operator profiles, but an opened session, an authorization handle, callback state, downstream bearer/refresh lineage, and transport cannot be recovered | **reset-by-design**: Stage 2 intentionally adds no aggregate/snapshot field, broker store, or reattachment path. A future composed process starts with no broker Runtime/session and requires a fresh explicit connection; it must never silently fall back to global direct MCP | ADR 0245 (Stage 2 proof) |

...
| # | Resource | Owner | Scope | Cleanup today | Re-attach | Evidence |
|---|---|---|---|---|---|---|
| 68 | Session-scoped MCP broker logical sessions, authorization transactions, grants/tokens, callback states, replay ledgers, and hardened OAuth HTTP client | `app.Built` owns one process-wide `internal/adapter/mcpbroker.Runtime` only for the local `mecated` compatibility path; standalone `cmd/mecabroker` owns the shipped remote-broker callback state, while `mecak8s` owns no ToolHive runtime or callback handler | process; logical state is partitioned by canonical session ID and private incarnation | `Built.Close` first bounds local attachment closure, then `Runtime.Close` fences the runtime, signals cancellation to every logical session, synchronously clears idle sessions, and returns without waiting on a stuck operation. No cleanup goroutine is launched: the registered operation's release path performs the one final secret/callback cleanup when the deleted/closed session reaches zero active operations. Permanent owner deletion/retention calls idempotent `DeleteSession`; ordinary `CloseSession` does not | **local runtime reset-by-design**: a local restart loses grants, transactions, callback/replay state, and logical incarnations. The persisted opaque binding then mismatches and reload fails closed. The remote-broker transport is shipped separately: its exact persist/reset/reattach decision is List 2 row 45 | `internal/adapter/mcpbroker/runtime.go` (`Runtime`, `DeleteSession`); `internal/adapter/mcpbroker/auth.go` (`Close`); `internal/app/build.go` (`Built`, `MountMCPBrokerHandlers`) |
| 69 | Service-local MCP broker attachment registry, per-attachment client-generation registry, shared client generation, replacement-admission fence, and per-session lifecycle serialization | `internal/adapter/server.Service` (`brokerAttachments`, `brokerAttachmentGeneration`, `brokerGeneration`, `brokerClosed`, `brokerMu`) | process / canonical session ID; wrappers are local handles, not logical broker state; the generation counter coalesces concurrent replacement attempts | attach/build/commit/abort, detach, and permanent deletion serialize per session without a process-wide bottleneck. A created attachment is provisional: host persistence calls attachment-scoped `Commit`; rollback calls `Abort`, which removes only a still-private creation and preserves a logical session once a peer has reattached. Failed rebound persistence removes the transient attachment and its generation entry. `Service.Close` permanently fences replacement admission before snapshotting the registries, so an in-flight replacement either finishes and joins that snapshot or no fresh client can publish after shutdown starts. `CloseSession` removes the engine, attachment, and generation entry before context-bounded close; `Service.Close` clears the registries, closes all attachments concurrently, then closes the published client generation under the replacement lock. Attachment close rejects new operations and waits context-sensitively for registered calls; it never deletes logical state | **derived/reset-by-design**: a local close/reload reattaches by exact persisted binding to the published generation. Process restart drops both generation counters, the shutdown fence, and the attachment registry; composition rebuilds generation 1 from the configured client with replacement admission open. A confirmed remote incarnation loss advances it, while a per-session binding mismatch adopts the current live generation without rotating it. See List 2 row 45 | `internal/adapter/server/service.go` (`Service`, `CloseSession`, `Close`, `keyedMutex`); `internal/adapter/server/mcp_broker.go`; `internal/mcpbroker/broker.go` (`SessionHandle`) |
...
| 72 | Standalone remote-broker admission gate, active-operation coordinator, public/admin listeners, and drain completion channel | `cmd/mecabroker` (`mcpbrokerserver.Production`) | broker process | preStop and SIGTERM share the once-guarded drain: admission closes before propagation wait, active work drains to its configured deadline, listeners stop, then verifier, ToolHive, and transport close; `Close` joins all owned sweepers | **reset-by-design**: a replacement starts a fresh singleton broker. It never transfers callback state, attachment handles, or active work; remote clients detect the new incarnation and fail protected continuations closed | `cmd/mecabroker/main.go` (`run`); `internal/adapter/mcpbrokerserver` (`Production`) |
| 73 | Remote broker attachment, logical-session, lifecycle/Execute receipt, pending-control, and active-Execute registries plus retention sweepers | `internal/adapter/mcpbrokergrpc.Server` | broker process / bounded by configured capacities and absolute deadlines | admission rejects new entries at capacity; `MaxActiveExecutes` reserves a process-wide slot before any goroutine or upstream dispatch, retains a bounded immutable capacity receipt on rejection, and releases the slot on every terminal/cancel/panic path; periodic sweeps reclaim expired unattached/idle state; server close cancels and joins active executions and stops sweepers before owned ToolHive resources close | **reset-by-design**: registries and active accounting deliberately do not survive broker replacement. A response lost after possible Execute remains ambiguous and is never replayed | `internal/adapter/mcpbrokergrpc/broker.go` (`Server`, `Config`, `Shutdown`) |
| 74 | mecak8s remote broker TLS client factory and projected audience-token reader | `cmd/mecak8s` composition | agent process / one factory per configured remote broker | each RPC rereads the bounded projected broker token; connection close is owned by the attachment/factory lifecycle; the Kubernetes API service-account token remains independently mounted for `SessionLease` | **reconstructible**: restart rebuilds the factory from the same CA and projected credentials. The remote broker is the authority; mecak8s persists no callback or ToolHive state | `internal/adapter/mcpbrokergrpc/factory.go` (`NewRemoteFactory`, `ProjectedTokenCredentials`); `deploy/helm/mecak8s/templates/deployment.yaml` |

**Remote-broker ownership re-audit (ADRs 0313–0315).** The standalone broker, not `mecak8s`, owns all browser callback routes and singleton process-local authority. Rows 72–73 capture the broker's bounded lifecycle and registry resources; row 74 records the separate agent-side client credential lifecycle. List 2 row 45 records the restart boundary: remote client transport reattaches only to a live matching incarnation, while broker owners, receipts, and authorization state reset fail closed.
...
| 45 | Remote MCP broker binding, pending authorization, and client-generation bookkeeping (ADRs 0313–0315; List 1 rows 69 and 72–74) | the snapshot persists the opaque `session.Session.ExternalBinding` plus the exact private `PendingAuthorization`; the standalone broker owns logical sessions, grants/tokens, callback/replay state, attachment handles, lifecycle/Execute receipts, and pending-control records in bounded process memory; mecak8s keeps only process-local shared/attachment generation counters | a mecak8s restart drops the generation counters, rebuilds its TLS client factory as generation 1, and reattaches only against the same live broker incarnation. A broker restart deliberately loses owners, handles, receipts, pending controls, runtime authorization/callback state, and grants; its new incarnation rejects the old binding, so the restored pending call is interrupted rather than replayed. Pre-prompt recovery advances the shared generation only for confirmed incarnation loss and re-derives each attachment generation; a stale per-session binding adopts the already-live generation. A lost Execute response remains ambiguous and is never automatically repeated | **persist opaque correlation / reset and re-derive generation/runtime state / reattach live transport**: the aggregate preserves exact effective and deferred calls, while generation counters, client connections, and all broker-owned authority are rebuilt only by a live authoritative broker. The counters are coalescing/fencing metadata, not authority and not snapshot fields. The public remote transport and its fail-closed restart behavior are shipped, not deferred | ADRs 0313, 0315 |

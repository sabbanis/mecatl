...
| 67 | Session-scoped read-before-write ledgers (ADR 0278) | each Workspace owns one selected `tool.ReadLedger`; default constructors own a fresh `memledger.Ledger`, while a `redisstore.Store` may vend durable handles bound to a session ID | session evidence; the Redis representation is one versioned hash per session and normalized path fields, independent of filesystem-content storage | in-memory ledgers die with their Workspace. Redis handles borrow the Store's client generations and add no client/goroutine; `Store.Close` owns connection cleanup. Session deletion/reuse must call idempotent `DeleteReadLedger` to delete the whole evidence scope; there is no TTL/age pruning and no production-default durable selection. Forkers always mint a fresh child memory ledger rather than inheriting the parent selection | default is **reset-by-design** and starts empty after rebuild; explicitly selected Redis is **persisted** across handles/processes until correctness cleanup. Storage/decode/corruption errors remain errors, never absence | `engine/tool/ledger.go` (`ReadLedger`); `engine/adapter/memledger/memledger.go` (`Ledger`); `internal/adapter/redisstore/readledger.go` (`ReadLedger`, `DeleteReadLedger`); `internal/adapter/forker/forker.go` (`Fork`) |

**Persistent read-ledger re-audit (List 1 / List 2 — ADR 0278).** List 1 row 67 inventories both selectable lifetimes without coupling either to filesystem-content storage. The memory default is a fresh session ledger and is lost safely on rebuild. Redis durability borrows the already-inventoried Store/client lifecycle (row 62), adds no connection or goroutine, and requires explicit whole-session cleanup before ID reuse. List 2 row 8 records the split fidelity decision: reset-by-design for the default, persist-in-store only when Redis is explicitly selected. Forks always select a fresh child memory ledger, and no production composition selects Redis by default.
...
| 8 | Workspace read-version ledger (ADR 0208, superseded for lifetime only by ADR 0278) | `tool.ReadLedger` selected per session and composed into each Workspace; default `memledger` is in-memory, while `redisstore.ReadLedger(sessionID)` is the durable adapter proof | default ledger is lost when its Workspace/process is rebuilt, so the next existing-file mutation is refused until Read records again. A selected Redis ledger reopens the exact opaque evidence across handles/processes, isolated by session hash; unavailable or corrupt storage is distinct from absence and fails closed | **independently selected**: reset-by-design for the fresh in-memory default; **persist-in-store** when the Redis ledger is explicitly selected. Redis lifecycle is borrowed from the containing Store, and session deletion/reuse must invoke idempotent `DeleteReadLedger`; TTL/age pruning and production-default wiring remain deferred. Forked Environments always get a fresh child in-memory ledger and never inherit parent evidence. The final filesystem `ReplaceFile` CAS remains unchanged | ADR 0278 (Accepted) |

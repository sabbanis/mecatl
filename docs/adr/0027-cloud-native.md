...
**MicroVM lifecycle re-audit (List 1 / List 2 — ADR 0108 Scenario 5).** The production
backend changes the earlier clean result: it adds the durable daemon registry and the
repository VM/process/socket/rootfs resource family plus logical attachment state (List 1
rows 73 and 79–80). The registry and attachment metadata hold lifecycle progress atomically
and are therefore also fidelity sources in List 2 rows 47 and 50. Harness-local environment handles remain reconstructible from the
session's existing exact `EnvironmentRef`; engine rehydration stays an independent decision.
Deletion and retention first persist a tombstone, then generation-fenced idempotent cleanup
converges after crashes or disk-full retries. PID plus process-start identity, VM id,
endpoint, and generation must all agree before destructive runtime work, so stale sockets,
PID reuse, and a second daemon cannot target another environment or resurrect a deleted one.

**Same-repository concurrency re-audit (Scenario 6).** The daemon-wide
`AdmissionController` adds process-lifetime per-owner/deployment counters and rolling boot
windows (List 1 row 74; List 2 row 48). Successful lifecycle creation retains one lease
covering the active VM, worktree, CPU, RAM, disk, and inode allocation; failed transactions
release it, and exec/fork/pull callers use the same atomic controller for their shorter-lived
leases. A daemon restart must rebuild active reservations from row 73's registry records plus
the operator profile before accepting creates. Exec/fork/pull reservations disappear with
their processes; rolling boot history resets at daemon restart by design.

**MicroVM operations re-audit (List 1 / List 2 — ADR 0108 Scenario 8).** The
process-scoped `OperationsObserver` adds one fixed-size in-memory metric snapshot
(List 1 row 76). It has no goroutine, descriptor, dynamic label set, or cleanup hook;
the composing daemon drops it at exit. The doctor is per-invocation and adds no
outlives-a-call resource. List 2 is unchanged: metrics are reset-by-design operator
observations, not session/run state, while stale-resource truth remains the existing
durable registry and attachment metadata in List 1 rows 73 and 79–80 / List 2 rows 47 and 50.

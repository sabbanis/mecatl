...
| 41 | Session-scoped MCP broker incarnation and local attachment (P10; List 1 rows 67–68) | the snapshot persists only `session.Session.ExternalBinding`; the process-owned broker holds logical state, grants/tokens, authorization transactions, callback/replay state, and the Service's wrapper attachment in memory | same-process local close/reload reattaches only when the opaque binding matches exactly. A process restart loses the logical incarnation and creates a new binding namespace, so a loaded old binding fails closed before catalogue assembly; authorization cannot be inferred or silently replaced | **reset-by-design for the P10 in-process proof, with an explicit fail-closed restart limitation**: durable/remote broker state must preserve or authoritatively remap the incarnation before restart reattachment can succeed. Public lifecycle and wire controls are later patches | P10 |

...
**Session-scoped vMCP broker re-audit (ADRs 0245 and 0248, Stage 3 command-root proof).** List 1
rows 64–68 record the optional root-internal Runtime, Service-owned session-local
transport wrappers, parked-completion health counter, durable pending authorization
correlation, per-session expiry timer ownership, and the process-local private ToolHive
auth-session cleanup identity/backlog. Row 27 remains the live lease
ownership and renewer: a second process fails closed while its holder is live, with no
routing or continuation handoff. List 2 row 40 records the opaque enrollment identity,
pending authorization snapshot, and deliberately restart-lost cleanup bookkeeping.
A restart rebuilds Runtime, transports, grants, callback transactions, and expiry workers
from scratch; it never reattaches a prior-process grant, catalogue, token-cleanup target, or
timer. Command roots mount the existing HandlerBundle only on their primary listener,
adding no listener or route-table resource.
...
| 64 | Optional session-scoped vMCP broker Runtime plus the Service registry of one `SessionTools` owner per enrolled persisted session (ADR 0245 Stage 3 proof) | root-internal `internal/adapter/vmcpbroker.Runtime`, constructed by `app.Build`; `mecated`/`mecak8s` mount the returned complete handler bundle on their existing primary HTTP mux | process / live Service session | `Service.CloseSession` and `Service.Close` remove and close the registered wrappers exactly once before Runtime close; `Built.Close` closes Runtime before profile resources | **derived + reset-by-design**: a restart constructs a new Runtime, validates the durable enrollment identity, then opens fresh transports; runtime grants and transactions are not persisted | `internal/app/build.go` (`Build`); `internal/adapter/server/service.go` (`openBrokerSession`, `CloseSession`, `Close`), `internal/adapter/vmcpbroker/runtime.go` (`Runtime`, `HandlerBundle.Mount`) |
...
| 67 | Pending vMCP authorization correlations and Service-owned fixed-expiry timers (`authorizationExpiry` pointer-entry map) | `engine/session.Session` durable pending state; `vmcpbroker.Runtime` exact transaction index; `server.Service` owns one timer callback per parked session | session / process | `FinishRun` installs the timer only for a durable parked outcome. The current pointer entry replaces and stops its predecessor before a new schedule; every resolution, lease loss, close, and shutdown stops/removes it. Its callback verifies pointer identity and Service closure before taking the ordinary lease-protected control path. Expiry drains its continuation through the normal relay, so EventLog append and live subscribers see the same events as a wire-driven continuation. No callback schedules work after Service shutdown. | **durable pending, reset runtime and timer**: a successor never treats a newly-created Runtime connection as continuity and interrupts the old parked operation rather than retrying it. The timer itself is process-local; a restart repair owns the persisted pending call and no stale prior-process callback survives. | `engine/session/mcp_authorization.go` (`PendingMCPAuthorization`, `InterruptMCPAuthorization`); `internal/adapter/server/mcp_authorization.go`; `internal/adapter/server/service.go` (`scheduleMCPAuthorizationExpiry`, `expireScheduledMCPAuthorization`, `stopMCPAuthorizationExpiry`); `internal/adapter/vmcpbroker/runtime.go` (`CancelAuthorization`) |
| 68 | Private ToolHive auth-session cleanup identities and failed-deletion backlog (`sessionLifecycle.authSessions`, `Runtime.tokenCleanup`) | `internal/adapter/vmcpbroker.Runtime`; identities are captured when callback grants commit and may cover multiple enrollment generations | session identities plus process-wide retry backlog | terminal ToolHive refresh failure and disconnect use provider-scoped deletion; `SessionTools.Close`/`ForgetSession` drain transport and lifecycle work before session-wide deletion, then clear live state even on cleanup failure; repeated Forget retries that owner's retained targets and Runtime close retries all. Cleanup is serialized separately from `Runtime.mu`, bounded, cancel-detached, and errors remain fixed/generic. Successful provider deletion clears every covered same-auth-session/provider backlog entry regardless of owner; successful session-wide deletion clears all entries for that auth session | **reset-by-design**: identifiers, provider keys, and cleanup failures are secret-shaped process-local custody and never enter snapshots/events/diagnostics. A crash can leave rows in ToolHive's process-owned memory storage, which dies with the embedded authserver; the next process requires fresh enrollment and has nothing to reattach | `internal/adapter/vmcpbroker/runtime.go` (`cleaningUpstreamTokens`, `recordAuthSessionLocked`, `cleanupTokenTargetsWithContext`, `ForgetSession`, `Runtime.Close`) |
...
| 41 | Session-scoped vMCP broker routes, authenticated frozen catalogue, session/backend bindings, pending callback transactions, downstream grants, disconnect marks, refresh operations, per-session streaming-HTTP clients, and private token-cleanup identities/backlog (ADRs 0245 and 0248; List 1 rows 64–68) | root-internal `internal/adapter/vmcpbroker.Runtime`; the aggregate persists only the opaque enrollment identity and pending MCP authorization needed for fail-closed correlation | process restart drops all Runtime state. Anonymous routes are re-discovered from operator profiles. Protected routes are rebuilt only after fresh bundled consent and provider-scoped authenticated `QueryCapabilities`; static `auth.oauth.tools` data cannot rebuild or override them. ToolHive's memory storage and its private cleanup targets die with the embedded authserver/process | **persist opaque correlation; reset private authority/custody by design**: no grant, provider key, auth-session ID, token, discovered protected catalogue, or cleanup error/backlog enters snapshot/event state. A replacement process requires fresh enrollment and never silently falls back to static or global direct MCP | ADR 0245; ADR 0248 |

...
yet) adds ONE new outlives-a-call resource (List 1 row 39): the token validator's
...
The re-audit verdict is CLEAN, with the caveat stated on row 39: the validator
that owns the JWKS cache is not yet wired in any main, so the row describes the
seam and its intended lifetime, not a resource this build allocates.
...
| 39 | OIDC token-validator JWKS cache + its background key-rotation refresh (caller identity, ADR 0100 decision 3/7; the `toolhive-core/authn` validator behind `server.PrincipalValidator`) | the VALIDATOR instance, constructed once per process by `internal/cliconfig/oidc.go` (`OIDCValidator`) and handed to `server.SecurityConfig.Validator`; mecatl owns the seam, never the JWT/JWKS mechanics | process (OPT-IN: only when `--oidc-issuer` is set; the zero value is identity OFF and allocates nothing) | the constructor is handed the SERVER-ROOT context, deliberately NOT a per-request one (a request-scoped ctx would tear key rotation down with the first request), so the root's cancel at shutdown stops the refresh; the refresh goroutine has no caller and therefore runs under the explicit system principal `mecatl:internal / jwks-refresh` (`internal/syscaller/syscaller.go` (`RootJWKSRefresh`)) | **reconstructible** (a restarted process re-resolves the flags and re-fetches the key set on the next Build; nothing is persisted and nothing should be — a cached signing key is a derivation of the IdP's live JWKS). NOTE, honestly: `OIDCConfig.NewValidator` is nil in both mains today because `toolhive-core/authn` does not exist yet, so this row describes the SEAM and its intended lifetime, not a resource this build allocates — setting `--oidc-issuer` today is a FATAL startup error, never a silent degrade to unauthenticated | `internal/cliconfig/oidc.go` (`OIDCConfig`, `OIDCValidator`, `RegisterOIDCFlags`); `internal/adapter/server/authn.go` (`PrincipalValidator`); `internal/syscaller/syscaller.go` (`RootJWKSRefresh`) |
...
| 26 | Session OWNER — the verified caller a session is attributed to (`session.Principal`, caller identity, ADR 0100 decision 4) | `engine/session/principal.go` (`Principal`, `RestoreLabels`): the `(iss, sub)` pair plus `GrantType`/`Name`, stamped WRITE-ONCE at create from the context principal (`internal/adapter/server/service.go` (`resolveOwner`), never from the request body) and inherited from the SOURCE session on a child/fork. Persisted as the additive `owner` snapshot field (`engine/adapter/sessnap/sessnap.go` (`Snapshot`), `omitempty`, pointer for true omission — no format-tag bump), restored by direct assignment through the aggregate method, so `RestoreState`'s signature is unchanged | survives restart, so a session listed or resumed after a restart still says who it belongs to; without it every pre-restart session would silently become unowned | **persist-in-snapshot** (additive `owner` — the identity-of-record for the session. `authority` rides alongside it as Track C's inert placeholder, round-tripped so the contended field exists before it is contended). Absent identity stays a nil owner: the edge NEVER fabricates a principal (`AGENTS.md` invariant, pinned by `TestInvariant_no_fabricated_principal`) | 0100 (issue #367) |
| 27 | `session.Event.Actor` — the log-only actor annotation on durable event-log records (caller identity, ADR 0100) | `engine/session/event.go` (`Event`): stamped at the RELAY, in the one place the durable log is written (`internal/adapter/server/service.go` (`appendEvent`), plus the `AppendRunEvent` delegate the scheduler's fire loop uses because it ranges `run.Events()` itself and is not a wire relay). The loop never sets it — it stays storage- and identity-agnostic | a restarted process re-derives it on every subsequent append from the session's own owner; historical records keep whatever they were stamped with | **derive-at-append**, explicitly NOT identity-of-record. The SESSION OWNER (row 26) is the record; `Actor` is an attribution annotation on log lines, so it is neither reconciled nor repaired on restart, and nothing may read it back as authority. It follows the `EvApproval` precedent: log-only, never relayed as a fact a decision hangs off | 0100 (issue #367) |
| 28 | Schedule OWNER (`port.ScheduleSpec.Owner`, caller identity, ADR 0100) | `engine/port/schedule.go` (`ScheduleSpec`): captured at CREATE from the creating caller's principal — never at fire time, which has no caller — and persisted with the schedule record across all four transports. A scheduled fire's `sched--` session carries the schedule owner with `GrantType: client_credentials` (the fire acts on the creator's behalf; it is not that human sitting at a terminal) | survives restart: a schedule created before a restart still fires as its creator afterwards. Deriving it at fire time was rejected — the fire loop runs under the scheduler's system principal (`internal/syscaller/syscaller.go` (`RootScheduler`)), so a fire-time derivation would attribute every scheduled session to `mecatl:internal` and lose the human who asked for it | **persist-in-schedule-record** (the create-time capture is the whole point; the fire path only reads it). `FireNow` is deliberately NOT wrapped in a system context — a manual fire keeps its requester's identity | 0100 (issue #367) |

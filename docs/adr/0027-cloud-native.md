...
file and reattaches a live `Environment` through the resolver. That phase originally added no
handle of its own. The later production MicroVM integration now retains the exact attached
binding for the Service/session lifecycle and shares it with long-lived borrowers; List 1 row 12
inventories that reconstructible cache. The in-tree Kinds never reach the resolver; a legacy
...
**MicroVM lifecycle re-audit (List 1 / List 2 - ADR 0349).** The production
backend adds the durable repository singleton record and the VM/process/socket/rootfs resource
family plus logical attachment records (List 1 rows 73 and 79-80). The singleton record and
attachment metadata are fidelity sources in List 2 rows 47 and 50. Harness-local environment
handles remain reconstructible from the session's existing exact `EnvironmentRef`; engine
rehydration stays an independent decision. Repository-VM replacement, deletion, and destructive
reconciliation remain deferred. Exact logical-attachment deletion removes clean worktrees or
persists dirty retention; generation and complete binding checks prevent targeting another
attachment.

**Same-repository concurrency re-audit (Scenario 6).** The repository-only lifecycle does
not include a per-session resource quota controller. The repository registry serializes first generation admission, while logical attachments and
forks remain distinct worktrees inside that one generation. Deployment-level capacity
policy is outside this repository-only lifecycle.

**MicroVM operations re-audit (List 1 / List 2 - ADR 0349).** The
process-scoped `OperationsObserver` adds one bounded in-memory metric snapshot and a
generation-keyed `egressSources` map (List 1 row 76). The map owns each active hosted-network
denial source and its last sampled counter until generation teardown removes it. It has no
goroutine or descriptor; the composing daemon drops the observer and any remaining sources at
exit. The doctor is per-invocation and adds no outlives-a-call resource. List 2 is unchanged:
metrics, source handles, and sample baselines reset by design, while repository and attachment
records remain the durable truth in List 1 rows 73 and 80 / List 2 rows 47 and 50.
...
| 12 | Service-owned exact placement attachments and per-session environments (`Service.attachedPlacements`, `Service.sessionEnvironments`, `Service.sessionEnvironmentCloses`: ordinary placement, no-FS, ACP buffers) | `server.Service` | exact placement generation shared by its owning sessions and long-lived team borrowers | a successful create transfers its detach callback only after the session snapshot is durable; `CloseSession`/`EndSession` release the session owner, team cleanup releases its borrow, and `Service.Close` drains both. Reference counting delays the single detach until the final borrower exits. A failed unpublished create invokes the binding's exact rollback instead; cleanup failure leaves the daemon's durable logical placement intact and emits an explicit recovery diagnostic | **derived from the exact durable ref**: the cache is process-local and resets on restart. The first borrower calls the provider's exact `Reattach`; later runs/discovery/ACP/team borrowers share that attached generation, never follow the current default, and never own independent detach callbacks. ACP may replace only the environment projection while retaining the same attachment owner | `internal/adapter/server/service.go` (`attachedPlacements`, `sessionEnvironments`, `sessionEnvironmentCloses`); `internal/adapter/server/placement.go` (`sessionPlacement`, `borrowSessionPlacement`, `rollbackUnpublishedPlacement`) |
...
| 73 | Repository-scoped local microVM placement: stable generation, replaceable VM/runner boot, private endpoint, retained rootfs, admitted launch artifacts, daemon service flock, launch receipts/locks, and registry flock (ADR 0349) | `mecatl-microvmd`; one `RepositoryVMRecord` per `(operator, canonical Git common directory)` in `RepositoryVMRegistry` | placement/rootfs/artifacts outlive sessions, daemon processes, and host boots; boot runtime is replaceable; the service flock spans one daemon process; registry and launch locks are operation/process scoped | first admission retains verified immutable artifacts, writes a provisioning intent with a unique host-only staging name, materializes there, atomically publishes one rootfs without overwrite, and commits ready after authenticated health. A retry rebuilds only the recorded never-published staging transaction or recognizes the already-published rootfs. The daemon acquires its stable owner-only service flock before socket cleanup or repository runtime mutation and retains it until exit. Recovery validates retained state and policy, reconciles every stable-repository launch attempt, rotates boot authority, and starts a new boot without changing the placement generation. Graceful shutdown stops owned VM/network resources but retains durable files | **persisted/reconstructible**: versioned registry, immutable artifact identities, rootfs, and launch intent/receipt/lock files survive restart. Exact launch reconciliation validates boot ID, process start, executable and lock-fd identity around pidfd open. Pending or uncertain ownership blocks replacement; no rootfs rematerialization or command replay occurs. See List 2 row 47 | `environment/microvm/repository_lifecycle.go` (`RepositoryVMRegistry`, `RepositoryVMRecord`, `Ensure`); `environment/microvm/repository_rootfs.go` (`repositoryRootFSMaterializer`); `environment/microvm/cmd/mecatl-microvmd/daemon_ownership_linux.go` (`acquireDaemonOwnership`); `environment/microvm/launch_ownership.go` (`LaunchOwnership`); `environment/microvm/launch_ownership_linux.go` (`Reconcile`) |
| 75 | repository-logical microVM child attachments and process-local fork/merge state (ADR 0349) | the authoritative `mecatl-microvmd` `RepositoryAttachmentManager` and `Daemon`; the root microVM client routes by the parent `EnvironmentRef.Kind` | each child attachment owns one logical worktree, exact repository generation binding, and guest registration; the active map holds fork parentage and base, and one process-local mutex serializes logical merges | Fork captures an immutable Git tree containing the parent's exact dirty state, creates a separate logical worktree in the same repository VM, and validates the complete returned owner/session/environment/ref/generation tuple. Merge checks process-local parentage, discovers all child changes, checks the parent against the fork base, validates the patch, immediately rechecks, and applies once. Conflict preserves the child. Cleanup uses a cancellation-detached bounded RPC and deletes only an exactly identified child attachment | **persisted/reset split**: attachment records persist the repository generation, exact binding, and worktree metadata. Active handles, fork parentage/base, and merge serialization reset with the daemon process. Exact reattachment rebuilds handles only against the retained repository generation; merge after process restart is unavailable. Crash-durable fork parentage and cross-process merge coordination remain deferred. See List 2 row 49 | `environment/microvm/repository_attachment.go` (`RepositoryAttachmentManager`, `Fork`, `Merge`, `register`); `environment/microvm/operational.go` (`captureForkBase`, `mergeRepositoryWorktrees`); `environment/microvm/daemon.go` (`Daemon`); `internal/adapter/microvm/client.go` (`Fork`) |
| 76 | Local microVM operations metrics and active egress-source snapshot (ADR 0349) | one `microvm.OperationsObserver` per daemon composition | process; fixed closed-dimension counters plus a generation-keyed map of active egress-denial sources and their last samples | no goroutine, fd, or timer. Generation registration adds or replaces one source after sampling its predecessor; snapshot samples each source; teardown samples and removes it. Producers may pass command/destination text only to methods that discard it immediately; diagnostics emit fixed messages and bounded dimensions. The observer and remaining map entries die with the daemon | **reset-by-design**: counters, source handles, and sample baselines are process observations, not session/run state. Durable repository and attachment truth remains in List 1 rows 73 and 80 / List 2 rows 47 and 50. The per-invocation Doctor adds no outlives-a-call resource | `environment/microvm/operations.go` (`OperationsObserver`, `trackEgressDenials`, `untrackEgressDenials`, `Snapshot`, `Doctor`) |
| 77 | Multiplexed microVM guest connection reader plus authentication/replay state (ADR 0349 data plane) | host `control.Client` owns one response-demultiplexing reader; generic guest `guestagent.Server` owns a bounded `CapabilityVerifier`, while `RepositoryServer` owns boot-authenticated registration sequence/incarnation state | live environment connection / guest-agent process | `guestagent.Services.Close` closes the sole stream and unblocks the reader. Generic environments retain at most 4096 consumed capability nonces and fail closed. Repository channels instead mutually authenticate the current boot, serialize lifecycle mutations with a monotonic sequence, retain one exact response for lost-acknowledgement replay, bind data claims to the current random registration incarnation, and cap persistent data at 16 slots plus one reserved control connection. A matching acknowledgement updates the named host registration incarnation even when another caller drains it; transport loss retains the exact pending request, while decoded semantic failure advances the sequence without applying success state. No host fallback exists | **derived/reset-safe**: generic reattachment mints a fresh capability; repository boot recovery rotates independently provisioned boot authority and reconstructs sequence/incarnation state. Earlier-boot proofs fail before either replay mechanism resets. No List 2 row | `environment/microvm/control/multiplex.go` (`Client`, `ServeMultiplex`); `environment/microvm/control/capability.go` (`CapabilityVerifier`); `environment/microvm/guestagent/guestagent.go` (`Server`, `Connect`); `environment/microvm/guestagent/repository.go` (`RepositoryServer`); `environment/microvm/repository_runtime.go` (`mutateLocked`) |
| 78 | Local microVM inventory-continuation signing key | one random 256-bit HMAC key owned by the authoritative `microvmd` `Daemon` | daemon process | no goroutine, fd, timer, collection, or explicit cleanup; the fixed-size key dies with the daemon. It authenticates bounded owner/cursor tokens and is never projected or persisted | **reset-by-design**: a daemon restart invalidates outstanding status continuations; the operator restarts pagination from the first deterministic owner-scoped page. Tokens are navigation state, not session/run state, so no List 2 row | `environment/microvm/daemon.go` (`Daemon`, `encodeInventoryCursor`, `decodeInventoryCursor`) |
| 79 | Repository-scoped microVM boot and logical-registration runtime | production `mecatl-microvmd` owns one `RepositoryComposition`; `RepositoryRuntime` owns current boot handles, capability issuers, authority copies, hosted network, and Unix listeners, while `RepositoryServer` owns the guest registration table | one replaceable boot under a stable repository placement; logical-environment lifetime for each registration and data stream | every connection proves the current boot authority over an unpredictable owner/repository/VM/boot-generation/purpose challenge before protocol payload. Boot-specific registrations use bounded monotonic mutation sequence and random incarnation, with exact lost-response retry. Graceful daemon shutdown stops VM/network resources under a bound and retains rootfs/worktrees/artifacts. On crash, launch ownership reconciliation prevents a second writer before replacement | **persisted/reconstructed split**: `RepositoryVMRegistry` persists stable placement plus current boot intent and immutable artifact/policy identities. A daemon restart reconstructs hosted networking and a new boot only after exact old-runner reconciliation, rotates authority, then recreates logical registrations from stable attachment refs. In-memory registrations, sequence state, listeners, and network handles reset. See List 1 row 80/List 2 row 50 for durable attachment inventory | `environment/microvm/repository_lifecycle.go` (`RepositoryVMRegistry`); `environment/microvm/repository_runtime.go` (`RepositoryRuntime`); `environment/microvm/repository_composition.go` (`RepositoryComposition`); `environment/microvm/guestagent/repository.go` (`RepositoryServer`); `environment/microvm/guestagent/repository_channel.go` (`AuthenticateHostRepositoryChannel`) |
| 80 | Repository logical-attachment operator inventory | one private versioned flat record `attachments/<logical-id>.json` sibling to the guest-exported `logical/` tree, plus an ignored private `attachments/.staging/` publication namespace, loaded into `RepositoryAttachmentManager.records` | logical attachment and retained-dirty recovery lifetime; the map is one daemon process cache | registration fsyncs owner/session/ref/stable placement generation, repository identity, confined worktree metadata, and deletion-retained state to a staging file, then atomically renames and fsyncs the committed-record directory. Clean close/delete atomically unlinks the flat record and fsyncs that directory, so interruption cannot expose an empty committed child directory. The document stores no rotating boot copy; live workspace/runner handles remain process-local | **persist-in-store**: a fresh manager ignores only validated private staging residue, scans every committed flat record, and loudly rejects corruption, symlinks, inconsistent identities, legacy guest-exported metadata, or the obsolete unmerged directory layout while preserving data. It resolves the registry's current boot record and reconstructs bounded status plus exact logical delete without booting. Ordinary exact reattachment may recover the runtime. Pagination signing tokens reset per row 78. See List 2 row 50 | `environment/microvm/repository_attachment_store.go`; `environment/microvm/repository_attachment.go` (`RepositoryAttachmentManager`) |
| 81 | Local microVM repository-capture admission and owner-private spool files | the `environment/microvm/worktree` package owns one process-wide two-slot semaphore; each admitted `Prepare` owns one private capture directory | semaphore for daemon process; spool files for one preparation transaction | acquisition is context-cancellable. One cumulative 256 MiB and 100,000-entry budget covers initial source-state content, patches, listings, and archive; verification scans use the same per-scan ceilings. Git output, dirty payloads, and tar extraction stream without whole-output buffering. The capture directory is removed on every return; worktree rollback removes provisional Git state after later errors | **reset-by-design**: the semaphore and temporary files carry no session state and reset or disappear with the process; successful registration occurs only after capture, exact verification, and temporary-file cleanup. No List 2 row | `environment/microvm/worktree/worktree.go` (`Prepare`); `environment/microvm/gitexec/gitexec.go` (`RunStream`) |
...
| 47 | Repository-scoped local microVM placement and boot record (ADR 0349; List 1 row 73) | `environment/microvm.RepositoryVMRegistry`: canonical owner/repository key, stable placement generation, private rootfs, retained artifact/policy identities, and replaceable boot intent | restart reloads the stable placement. A healthy same-daemon boot is reused; otherwise exact launch receipts and locks reconcile the old runner before authority rotation and a new boot. Provisioning retries use the same rootfs and artifacts. Missing, malformed, incompatible, pending, or uncertain state fails without replacement or deletion | **persist-in-store/reconstruct boot**: one owner/repository-confined, flock-serialized versioned JSON record plus retained artifact trees and launch-attempt records. Registry and authority commits use fsync and atomic rename. Concurrent recovery converges under the same key lock; session and schedule refs retain the stable generation | 0349 |
| 49 | Repository-logical child identity, immutable exact-tree fork base, and process-local merge synchronization (ADR 0349; List 1 row 75) | child binding and repository/worktree metadata persist in the private attachment record; the active `RepositoryAttachmentManager` map alone holds parent ref and fork base, while the daemon owns one merge mutex | restart loses active guest handles, fork parentage/base, and mutex state. Persisted attachment identity remains available for exact status, delete, and reattachment against the retained repository generation, but does not authorize merge | **persist/reset split**: exact attachment identity survives, while fork relationship and merge authority reset by design. A restarted process cannot merge the child. Crash-durable fork parentage and cross-process merge coordination remain deferred. Cancellation cleanup is detached and bounded; malformed fork responses never authorize automatic deletion because the returned tuple does not prove request-scoped creation. The caller receives guidance to inspect status and explicitly delete the intended attachment. | ADR 0349 |
| 50 | Repository logical-attachment inventory and dirty-retention decision (List 1 row 80) | host-only private versioned metadata stores exact owner/session/ref/stable generation, repository identity, source/worktree/metadata/branch paths, and deletion-retained state; the manager map is derived | restart drops live guest registrations and Workspace/runner handles but reloads status/delete identity without booting. The document resolves the registry's current boot instead of persisting a stale boot copy. A dirty retained worktree remains available for exact recovery or cleanup | **persist-in-store**: fsync + atomic rename + directory fsync under the host-only `attachments/` namespace outside the guest export. Startup rejects legacy exported metadata, malformed, symlinked, duplicate, or path-inconsistent records. Exact delete does not require guest reattachment; repository-VM deletion and a retention reconciler remain deferred | Repository inventory correction |

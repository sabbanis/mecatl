...
**Permanent-provider-error re-audit (List 1 / List 2 — issue #346, ADR 0203).**
The permanent-provider-error signal (ADR 0203) added NO new outlives-a-call
...
| 61 | Local Project documents and immutable canonical-source registry (issue #620, ADR 0234) | `app.Build` wires `projectstore.Local` and `projectsource.CanonicalRoot` only for ownerless `mecated --store-dir` deployments | durable Project documents under `<store-dir>/projects`; one immutable source registration per Build | records use the adapter's flocked create/CAS/replace/delete protocol; the registry owns no goroutine, connection, or mutable cache and is discarded with Build | **persisted / reconstructed**: Project documents reload from the same StoreDir; the registry deterministically re-derives its opaque source reference from the configured canonical root. A changed/missing root produces a different/absent registration and later Project Session resolution fails closed | `internal/adapter/projectstore/local.go` (`Local`); `internal/adapter/projectsource/canonical_root.go` (`CanonicalRoot`); `internal/app/build.go` (`buildLocalProjectControl`) |
...
| 38 | Captured Project binding and working `EnvironmentRef` (issue #620; List 1 row 61) | `session.Session.Project` plus the existing persisted `EnvironmentRef`; created only by the server Project control plane | a Project Session snapshot preserves its Project ID/name/revision, opaque source ref, safe working label, and exact environment identity; the live local environment override is process-local | **persist-in-snapshot / explicit reattachment**: later lifecycle work resolves the captured source registration and reattaches the exact EnvironmentRef without rereading the mutable Project. A missing or changed registration fails closed; the Project document itself is not needed to continue an existing Session | ADR 0234 |

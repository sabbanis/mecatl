...
| 28 | MCP standalone-SSE listener goroutine (`handleSSE`) per connected server — ONLY when the stream is enabled (the default; a server configured with `ServerConfig.DisableNotifications` / `MCP_<NAME>_DISABLE_NOTIFICATIONS`, ADR 0326, never opens it) | `mcp.Server` | per connected server (rides the SDK session, opened after `initialize` when `DisableStandaloneSSE: false`) | `Server.Close()` → `session.Close()` → `conn.Close()` cancels `connCtx` → `handleSSE` returns (async; the `mcp` package's `goleak` gate has a targeted ignore list for the SDK + stdlib goroutines that unwind asynchronously after close) | none (the SDK reconnects the stream itself on a transient drop; #177/ADR 0056 reconnects the whole session when the SSE reconnect exhausts → `ErrSessionMissing`) | `internal/adapter/mcp/mcp.go` (`dial`); ADR 0057, ADR 0326 |
| | _(traceability)_ ADR 0327 adds `mcp.Server.sseHealth` (`sseHealthTracker`): a mutex-guarded counter plus an `atomic.Bool` verdict, no goroutine, that can ALSO drive this row's `DisableStandaloneSSE` gate — automatically, once it observes 3 consecutive zero-byte fast GET closes — alongside the existing manual `ServerConfig.DisableNotifications`. **No new resource row**: it is in-memory state on the already-inventoried `mcp.Server`, not a new goroutine/cache/lease; **reset-by-design** on restart (a fresh process re-probes from zero, no persisted verdict — the sticky-for-process-lifetime contract is deliberately NOT a sticky-forever one) | | | | | `internal/adapter/mcp/ssehealth.go`; ADR 0327 |

...
| 70 | Managed temporary namespace/workspace handles, command/job reaper worker, root GC lock/completion record, and per-lease locks (ADR 0281) | `app.Build` / `managedtemp.Namespace` | process handles and worker; durable private managed root | `Built.Close` cancels the worker and waits only `shutdown_reap_timeout`; each startup/periodic sweep is bounded by `reap_timeout`. Namespace recovery and operation-handle pinning share a mutex: reopening the exact configured private root retires the stale cached handle, while per-operation pins close on return and independently owned workspace/lease handles are never closed or rehomed beneath active users. Runner construction opens and immediately closes a validation workspace; each command/job allocation reopens through namespace recovery and its lease owns the workspace until `Lease.Close` (including failed removal). There is no Build-lifetime workspace registry. Writable child provider guards borrow the existing Config/provider without new handles or session state, revalidating configured shell construction before model invocation; fork builders propagate construction errors before driving a child. The root GC lock is non-blocking, lease locks are acquired non-blockingly per candidate, and only validated unlocked leases past TTL are removed. System or unsupported modes construct no worker and make no mutation | **mixed**: the ticker/worker reset on restart; owner-only completion record and lease manifests persist. Missing/replaced namespaces are revalidated and safely reopened or recreated on later opens/sweeps; unsafe replacements fail closed, and configured local Bash placement cannot silently become shell-less. Old allocations whose visible workspace was replaced fail rather than being adopted into the new namespace. A crashed holder releases advisory locks, so a later interval-gated Build sweep can reclaim eligible residue; incomplete/cancelled scans publish no completion record. No List 2 row: this is adapter-owned cleanup coordination, not session/run state | `internal/app/managed_temp_worker.go` (`startManagedTempWorker`); `internal/app/temporary_storage.go` (`managedTemporaryStorage`); `internal/adapter/managedtemp/namespace_unix.go` (`openRoot`, `refreshLocked`, `Close`); `internal/adapter/managedtemp/reaper_unix.go` (`Sweep`) |

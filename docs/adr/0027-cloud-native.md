...
| 73 | Remote broker attachment, logical-session, lifecycle/Execute receipt, and pending-control registries plus retention sweepers | `internal/adapter/mcpbrokergrpc.Server` | broker process / bounded by configured capacities and absolute deadlines | admission rejects new entries at capacity; periodic sweeps reclaim expired unattached/idle state; server close stops sweepers before owned ToolHive resources close | **reset-by-design**: registries deliberately do not survive broker replacement. A response lost after possible Execute remains ambiguous and is never replayed | `internal/adapter/mcpbrokergrpc/broker.go` (`Server`, `Config`, `Close`) |

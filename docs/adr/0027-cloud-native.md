...
| 18 | A subagent delegation's ROUTED model (ADR 0031) | nowhere — the model the router chose for a child is NOT persisted; the child SESSION persists (row 7), but which model it ran on is a per-delegation, decide-once classification | a NEW plain delegation classifies fresh (the classifier is cheap); a `resume` is NOT re-classified — the router gates on `!resuming`, so a resumed child stays on the DEFAULT explorer engine (the existing `resume:` policy, unchanged by ADR 0031). No fidelity gap: the route is advisory model selection, never correctness | derive (a fresh delegation re-classifies on demand; a resume is never routed) — no new snapshot field | ADR 0031 (decide-once, derived) |

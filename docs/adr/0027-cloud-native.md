...
64–67 record the optional root-internal Runtime, the Service-owned, session-local
transport wrappers, the reset-by-design parked-completion health counter, the durable
pending authorization correlation, and per-session expiry timer ownership. A durable opaque enrollment identity admits rehydration only when

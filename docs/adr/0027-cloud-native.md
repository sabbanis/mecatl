...
| 68 | Session-scoped read-before-write ledgers (ADR 0289) | each Environment carries one selected `tool.ReadLedger` independently from its content-only Workspace; default composition owns a fresh `memledger.Ledger`, while a `redisstore.Store` may vend durable handles bound to a session ID | session evidence; the Redis representation is one versioned hash per session and normalized path fields, independent of filesystem-content storage | in-memory ledgers die with their Environment. Redis handles borrow the Store's client generations and add no client/goroutine; `Store.Close` owns connection cleanup. Both canonical session-deletion scripts atomically delete the hash with the other sidecars; `DeleteReadLedger` remains an idempotent ledger-only reset. There is no TTL/age pruning and no production-default durable selection. Every child gets a fresh memory ledger; direct-write/base-sharing children retain the exact parent content backend and runner through any stricter child-authority Workspace view, without reconstructing storage from `Root()` | default is **reset-by-design** and starts empty after Environment rebuild; explicitly selected Redis is **persisted** across handles/processes until correctness cleanup. Storage/decode/corruption errors remain errors, never absence | `engine/tool/environment.go` (`Environment`); `engine/tool/ledger.go` (`ReadLedger`); `engine/adapter/memledger/memledger.go` (`Ledger`); `internal/adapter/redisstore/readledger.go` (`ReadLedger`, `DeleteReadLedger`); `internal/adapter/redisstore/metadata_index.go`; `internal/adapter/forker/forker.go` (`Fork`) |

**Persistent read-ledger re-audit (List 1 / List 2 — ADR 0289).** List 1 row 68 inventories both selectable lifetimes without coupling either to filesystem-content storage. The memory default is a fresh session Environment capability and is lost safely on rebuild. Redis durability borrows the already-inventoried Store/client lifecycle (row 62), adds no connection or goroutine, and is atomically deleted by canonical session cleanup. List 2 row 8 records the split fidelity decision: reset-by-design for the default, persist-in-store only when Redis is explicitly selected. Every child receives a fresh ledger without changing its selected content backend, and no production composition selects Redis by default.
...
| 8 | Environment read-version ledger (ADR 0208, superseded for lifetime/ownership only by ADR 0289) | `tool.ReadLedger` is a separate mandatory Environment capability beside the content-only Workspace; default composition supplies `memledger`, while `redisstore.ReadLedger(sessionID)` is the durable adapter proof | the default ledger is lost when its Environment/process is rebuilt, so the next existing-file mutation is refused until Read records again. A selected Redis ledger reopens exact opaque evidence across handles/processes, isolated by session hash; unavailable or corrupt storage is distinct from absence and fails closed | **independently selected**: reset-by-design for the fresh in-memory default; **persist-in-store** when Redis is explicitly selected. Redis lifecycle is borrowed from the containing Store, and both canonical session-deletion scripts atomically remove its hash; TTL/age pruning and production-default wiring remain deferred. Every child gets a fresh ledger, while direct-write/base-sharing children retain the exact parent content backend and runner through any stricter child-authority Workspace view. Final filesystem `ReplaceFile` CAS remains unchanged | ADR 0289 (Accepted) |

...
| 67 | Pending vMCP authorization correlations and fixed expiry observations | `engine/session.Session` durable pending state; `vmcpbroker.Runtime` exact transaction index; `server.Service` control path | session / process | exact cancellation, expiration, session close, and Runtime close remove the Runtime correlation; durable resolution pairs the parked calls before ordinary continuation | **durable pending, reset runtime**: a successor never treats a newly-created Runtime connection as continuity and interrupts the old parked operation rather than retrying it | `engine/session/mcp_authorization.go` (`PendingMCPAuthorization`, `InterruptMCPAuthorization`); `internal/adapter/server/mcp_authorization.go`; `internal/adapter/vmcpbroker/runtime.go` (`CancelAuthorization`, `collectExpiredLocked`) |

...
| 29 | guardrail one-shot override (`OverrideArmer`, ADR 0061) | `app.Build` constructs; `server.Service` arms (`StartRunContent` genuine-prompt scan), `modelhook.Runner` consumes (`blockOrOverride`) | process | self-clearing via the one-shot atomic `Consume`; dies with the process (no `Close` — a nil `*OverrideArmer` is the byte-identical OFF posture) | **lost** (in-memory; an armed-but-unconsumed override never silently survives restart — fail-safe: the call re-blocks until a genuine human prompt re-arms it, ADR 0061) | `internal/adapter/modelhook/override.go` (`OverrideArmer`); armed in `internal/adapter/server/service.go` (`StartRunContent`); constructed in `internal/app/build.go` |
...
| 20 | guardrail one-shot override (`OverrideArmer`, ADR 0061) | process-lifetime in-memory map on `server.Config.OverrideArmer` (`internal/adapter/modelhook/override.go`; wired in `internal/app/build.go`, armed in `internal/adapter/server/service.go`) | an armed-but-unconsumed override is lost on restart | reset-by-design (fail-safe; a restarted session re-blocks until a genuine human prompt re-arms it — the override arms ONLY from the genuine pre-expansion/pre-injection user prompt, ADR 0061) | 0061 |

...
|---|---|---|---|---|---|---|
| 73 | Contextual guardrail transient review-detail registry | `server.Service` (`ReviewDetailRegistry`) | process registry, entries bound to one delegation-root run | root-run finish/cancel/disconnect clears that root; `CloseSession` and `Service.Close` clear applicable/all entries. Admission is bounded before retained insertion to 4,096 entries and 16 MiB, with each input field pre-clamped to 4 KiB and each rendered field to 1,000 runes | **reset-by-design**: live human display only; restart intentionally returns not found and durable machine events retain no prose | `internal/adapter/server/guardrails_status.go` (`ReviewDetailRegistry`, `PublishReviewDetail`, `clear`, `clearAll`) |
| 74 | Contextual guardrail held-result escrow map | delegation-root `agent.Run` (`reviewRoot`) | root run; exact repaired result retained only while a live `result_release` ask is resolvable | release/deny consumes or destroys one entry; cancellation and both run termination paths clear all. Admission checks 32-result / 2 MiB aggregate capacity before cloning; exhaustion withholds with a synthetic paired error | **reset-by-design, fail-closed**: bytes are never snapshotted or logged; restart records the lost-held-result synthetic error and never reruns the tool | `engine/agent/inboundreview.go` (`holdResult`, `consumeHeld`, `clearHeld`); `engine/agent/loop.go` (`startRun`) |
| 75 | Contextual review evidence handles and bounded previews | one `contextualToolReviewer.Review` / `finiteReviewEvidenceSource` | review attempt inside a root run | source is closed after review; finite opaque page chains share a source version and are limited to 16 handles and 400,000 aggregate bytes. Backend size/capacity preflight precedes handle allocation/content reads; each page is bounded to 25,000 bytes / 2,000 lines, and an acceptable assessment after starting a chain requires every page | **reset-by-design**: opaque capabilities and preview bodies expire with the review and are never persisted; a restarted review must derive fresh authorized evidence or resolve incomplete | `internal/app/contextual_reviewer.go` (`finiteReviewEvidenceSource`, `newFiniteReviewEvidenceSource`, `ReadReviewEvidence`, `CloseReviewEvidence`) |
| 76 | Contextual guardrail current-root trajectory ledger | delegation-root `agent.Run` (`reviewRoot`) | root run shared with its workers | dies after child drain/run termination; exact duplicates are skipped. Admission checks 256 facts / 128,000 retained bytes before append; exhaustion sticks `TrajectoryComplete=false` and retains only bounded authorization-class metadata when it fits | **reset-by-design**: no cross-run/session/restart authority; missing trajectory forces a capacity-dependent assessment unresolved rather than acceptable | `engine/agent/actionreview.go` (`reviewRoot`, `record`, `snapshot`); `engine/agent/loop.go` (`startRun`) |
| 77 | Contextual guardrail exact repeat grant registry and HMAC key | `app.Build` (`modelhook.WaiverHolder`) | process, partitioned by session | `SessionCleared` calls `ClearSession`; process shutdown destroys key and map. Admission is bounded to 1,024 sessions and 256 digests/session; full capacity makes repeat unavailable while Run once remains | **reset-by-design, fail-safe**: grants and random key never persist; restart or session clear requires a fresh contextual review/human approval | `internal/adapter/modelhook/waiver.go` (`WaiverHolder`, `CanArm`, `ClearSession`); `internal/app/build.go` (`SessionCleared`) |
| 78 | Plan approval receipt registry | `app.Build` (`planApprovalReceipts`) | process, at most one exact `{ref, session, call, target mode}` receipt per session and 1,024 sessions total; absent when the checker is unconfigured or disabled | consumed or discarded once after the next accepted execution prompt; both `OnCloseSession` and `SessionCleared` remove the session entry; process shutdown drops the map | **reset-by-design, fail-safe**: restart never reconstructs plan approval authority from snapshots, events, or prose | `internal/app/plan_approval_receipts.go` (`planApprovalReceipts`, `RecordPlanApproval`, `ConsumePlanApproval`, `ClearPlanApprovals`); `internal/app/build.go` (`OnCloseSession`, `SessionCleared`) |
...
| 21 | contextual guardrail checker-route health (`guardrailRouteHealth`) | `app.Build` | process | dies with the Build | **lost** (in-memory; starts as not-yet-checked after restart) | `internal/app/guardrails.go` (`guardrailRouteHealth`) |
...
| 29 | guardrail session exact-action grant store (`WaiverHolder`) | `app.Build` constructs; contextual action review arms it after a human repeat approval | process | self-clearing; dies with the process | **lost** (in-memory; a grant never silently survives restart) | `internal/adapter/modelhook/waiver.go` (`WaiverHolder`); armed via `internal/app/guardrails.go` (`ArmGrant`); constructed in `internal/app/build.go` |
...
|---|---|---|---|---|---|
| 47 | Contextual guardrail transient review detail | process-local `server.ReviewDetailRegistry`, bound to an exact root/child relation and review id | all concern/source/next-action prose disappears | **reset-by-design**: this is an owner-authorized live display channel, not durable audit; restart truthfully returns not found while machine-only review state remains in events | ADR 0350 |
| 48 | Contextual held-result escrow | root-run `agent.reviewRoot.held`, paired to the live ask and exact session/call/environment | already-produced unreleased bytes disappear and cannot be reconstructed | **reset-by-design, fail-closed**: awaiting restart closes the call with an accurate synthetic lost-held-result error; never re-execute a side effect to recover bytes | ADR 0350 |
| 49 | Contextual review evidence capabilities/previews | review-local `app.finiteReviewEvidenceSource` | handles, bindings, read state, and preview bodies disappear | **reset-by-design**: a future review mints new finite capabilities only from current authorized sources; unavailable evidence makes the assessment incomplete/unresolved | ADR 0350 |
| 50 | Contextual current-root trajectory | root-run `agent.reviewRoot.facts` plus completeness/byte accounting | the run and worker correlation ledger disappears | **reset-by-design**: its authority is deliberately one delegation-root run; no cross-session or restart inference is permitted | ADR 0350 |
| 51 | Contextual exact repeat grants | Build-owned `modelhook.WaiverHolder` map and random HMAC key | every digest and the key disappear | **reset-by-design, fail-safe**: a restarted process/session cannot match the old grant and requires fresh review/human approval; it is not permission learning or approval replay | ADR 0350 |
| 52 | Plan approval receipt | Build-owned `app.planApprovalReceipts`, exact ref/session/call/target mode | every unconsumed receipt disappears | **reset-by-design, fail-safe**: restart cannot reconstruct action authority; only a new real plan callback can mint another receipt | ADR 0350 |
...
| 15 | contextual guardrail checker-route health | process-wide on `guardrailRouteHealth` (`internal/app/guardrails.go`) | resets to not-yet-checked | reset-by-design (status/UX only; runtime enforcement uses each review outcome) | n/a |
...
| 20 | guardrail session exact-action grant store (`WaiverHolder`) | process-lifetime in-memory map (`internal/adapter/modelhook/waiver.go`; constructed in `internal/app/build.go`, armed through `internal/app/guardrails.go` (`ArmGrant`)) | a session grant is lost on restart | reset-by-design (fail-safe; a restarted session re-asks until a human re-approves) | 0062 |

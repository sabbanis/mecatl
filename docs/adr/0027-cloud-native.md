...
| 45 | Remote MCP broker binding and pending authorization (ADRs 0313–0315; List 1 rows 72–74) | the snapshot persists the opaque `session.Session.ExternalBinding` plus the exact private `PendingAuthorization`; the standalone broker owns logical sessions, grants/tokens, callback/replay state, attachment handles, lifecycle/Execute receipts, and pending-control records in bounded process memory | a mecak8s restart rebuilds its TLS client factory and reattaches only against the same live broker incarnation. A broker restart deliberately loses owners, handles, receipts, pending controls, runtime authorization/callback state, and grants; its new incarnation rejects the old binding, so the restored pending call is interrupted rather than replayed. A lost Execute response remains ambiguous and is never automatically repeated | **persist opaque correlation / reset remote runtime / reattach live transport**: the aggregate preserves exact effective and deferred calls, while client connections and all broker-owned authority are rebuilt only by a live authoritative broker. The public remote transport and its fail-closed restart behavior are shipped, not deferred | ADRs 0313, 0315 |

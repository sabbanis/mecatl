...
| 50 | Contextual current-root trajectory | root-run `agent.reviewRoot.facts` plus completeness/byte accounting | the run and worker correlation ledger disappears | **reset-by-design**: its authority is deliberately one delegation-root run; no cross-session or restart inference is permitted | ADR 0342 |
| 51 | Contextual exact repeat grants | Build-owned `modelhook.WaiverHolder` map and random HMAC key | every digest and the key disappear | **reset-by-design, fail-safe**: a restarted process/session cannot match the old grant and requires fresh review/human approval; it is not permission learning or approval replay | ADR 0342 |

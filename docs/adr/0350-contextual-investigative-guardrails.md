...
The harness supplies immutable provenance for the configured last one to three explicitly authenticated root user tasks (default one), genuinely admitted project instructions, exact positive plan callbacks, caller capabilities, environment identity, and relevant current-root trajectory. Only accepted root prompt/steer aggregate ingress marks principal provenance; worker prompts and steers, ordinary and legacy user-role records remain unknown, harness continuations are non-principal, and no title/content heuristic can promote any of them. Unknown provenance makes authority incomplete only while it could hide one of K requested tasks; the reviewer receives a bounded unavailable-provenance fact naming recovery: submit enough genuine root messages to fill K, or choose `taskWindow=1`. K later known tasks restore completeness. Principal snapshots carry a synchronized monotonic revision. Each action crosses a short root-mutex admission that atomically checks this revision and is the execution linearization point: a change committed before admission forces fresh review, while a later change does not retroactively cancel an in-flight action. The mutex is not held across evidence preparation, human/child waits, dependency reads, or tool execution. Post-action repeat publication separately requires the recomputed revision to equal the approved revision and atomically couples its final revision check to the bounded grant-map update. A plan callback crosses the plan terminal only as one exact, process-local, single-use `{ref, session, call, target mode}` receipt consumed or discarded after the next accepted execution prompt; result release and permission/repeat grants are distinct authority classes. Repeat digests bind canonical ordered principal facts and completeness in addition to call/dependency scope. Text remains untrusted; provenance is not inferred from filenames, domains, delivery notes, or words such as “approved.” Workers receive bounded harness facts, not parent transcripts.
...
Guardrails remain independent from permission modes and the automation posture: neither an allow nor `auto`/`yolo` enables a checker or authorizes checker spend. Startup labels `auto` without a resolved checker as `UNSUPERVISED`. Mecatui `/posture` reuses the existing owner-authorized coverage RPC to report checker off with setup guidance, advisory/enforcing when on, and unknown when an older or unavailable server cannot establish state.

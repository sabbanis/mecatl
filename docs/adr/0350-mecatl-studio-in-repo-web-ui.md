# ADR 0350 — Mecatl Studio: in-repo web UI behind a BFF over the published SDK
...
- Committed to: publishing the image from the first tag with an experimental label while
  the skeleton has no features, so infrastructure can wire the deployment early.

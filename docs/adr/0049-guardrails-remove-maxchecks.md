...
- The `checkBudget` type and its `admit` method are removed from `internal/adapter/modelhook/breaker.go`. The `failureStreak` (the consecutive-checker-FAILURE escalation to the one-time "checker DOWN" sticky WARN) is KEPT unchanged — it bounds a correctness concern (a persistently-broken checker), not a cost concern.

...
- Superseded by: [ADR 0350](./0350-contextual-investigative-guardrails.md) for contextual coverage and fixed-rubric/additive-policy behavior
...
ADR 0350 retains per-rule `Prompt` only as additive operator task-risk context beneath the fixed harness safety, authority, provenance, evidence, and structured-output rubric. It no longer replaces the rubric, and the broader contextual default set now covers local action and inbound boundaries.

# ADR 0328 — Bounded singleton MCP broker correctness
...
ADR 0326 selected process-bound remote attachments and prohibited application retries after
...
values that did not represent the client and network boundaries described by ADR 0327.
...
ADR 0327 remains the topology decision; this amendment completes its resource inventory and the
...
- [ADR 0326 — Process-bound remote MCP broker attachments](./0326-process-bound-remote-mcp-broker.md)
- [ADR 0327 — Single-replica production topology](./0327-single-replica-mcp-broker-topology.md)

...
`org.stacklok.mecatl.studio.stage=experimental` while the skeleton has no product
features; `apps/docker-compose.yml` runs it against a locally built `mecated` for

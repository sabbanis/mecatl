...
### Session-scoped vMCP broker (Stage 2 integration proof)

`internal/adapter/vmcpbroker` is a root-internal, process-lifetime broker runtime
for the separate brokered-MCP path in [ADR 0237](adr/0237-session-scoped-vmcp-broker.md).
It owns its immutable route catalogue, embedded ToolHive authserver/vMCP resources,
and only broker-private session/backend bindings. `OpenSession` returns a fresh,
session-local wrapper set; the wrapper retains the route privately and presents only
an ordinary `tool.ToolSpec` to the engine. A protected route stays in that fixed
catalogue before connection and returns a bounded authorization-required tool error
until a trusted composition caller invokes `Connect(sessionID, backendID)`.

The runtime is deliberately not an engine port, is not snapshot state, and does not
put a session ID, backend ID, bearer, ToolHive locator, or OAuth value in model-facing
arguments, results, events, or headers. `SessionTools.Close` drains its calls and
then forgets only that session; `Runtime.Close` drains session tools and closes the
shared ToolHive resources. Restart drops every broker binding and downstream grant;
there is no restart reattachment. The current implementation is a root-internal,
hermetic composition proof, not yet wired into `app.Build`, a server control endpoint,
or mecatui. Therefore no deployed root exposes broker tools or the private control
operations yet. The global MCP manager and its direct-MCP OAuth path are unchanged.

Stage 2 supports one protected upstream lineage. A disconnected protected backend and
a second protected backend fail with the explicit `ConnectUpstream` capability
limitation instead of creating another lineage. ToolHive v0.40.0 owns upstream OAuth;
its default upstream HTTP client has no mecatl injection seam, so this path does not
claim mecatl-enforced proxy, DNS-pinning, or redirect policy. The evidence and
remaining manual boundary are recorded in [`STAGE2-RESULTS.md`](../STAGE2-RESULTS.md).

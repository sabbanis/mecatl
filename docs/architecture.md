...
SBOM, and provenance steps as the Slack bot image; `apps/docker-compose.yml` runs it
against a locally built `mecated` for development. CI gates it with its own `studio` job and path-relevance category
...
`run.event`, `run.error` — where `run.event` wraps the SDK event with `bigint` counters
...
retried. See
[ADR 0350](adr/0350-mecatl-studio-in-repo-web-ui.md), the

...
### Session-scoped vMCP broker (Stage 3 command-root proof)

`internal/adapter/vmcpbroker` is a root-internal, process-lifetime broker runtime
for the separate brokered-MCP path in [ADR 0245](adr/0284-session-scoped-vmcp-broker.md).
`app.Build` returns its complete `HandlerBundle`; only `mecated` and `mecak8s`
mount that bundle with `HandlerBundle.Mount` on their existing primary HTTP mux and
listener. No second listener and no duplicate broker route table exists. The fixed
broker authorization, token, discovery, vMCP, and exact configured callback routes
remain separate from API, health, drain, metrics, and control routes.

A network-reachable broker control plane requires verified caller identity so durable
session ownership is enforced. Protected broker profiles use OIDC discovery by default.
A broker-only explicit `oauth2` upstream instead requires exact canonical HTTPS
authorization and token endpoints; GitHub remote MCP is the motivating interoperability
case. Generic OAuth2 is rejected by global/direct MCP, so its existing OAuth controller,
login, credentials, and network behavior do not change. Because ToolHive does not expose
the required upstream HTTP seam, explicit OAuth2 also rejects non-empty additional/private
origins and non-zero redirect limits instead of claiming to enforce them. Client secrets
remain environment references, never values in operator YAML.

Ownerless controls are allowed only for mecated's
explicit loopback single-user composition; mecak8s never permits ownerless controls,
even on loopback. Broker mode supports at most one protected backend and is a
single-process/single-replica proof: a non-holder
of a live session lease fails closed with `leased elsewhere`; it does not route,
interrupt, recheck, or execute on the holder's behalf. Restart rebuilds Runtime state
and fresh session wrappers after enrollment validation; grants, transactions, and
transports do not reattach. See [`STAGE3-RESULTS.md`](../STAGE3-RESULTS.md).

...
session ownership is enforced. Ownerless controls are allowed only for mecated's
explicit loopback single-user composition; mecak8s never permits ownerless controls,
even on loopback. Broker mode is a single-process/single-replica proof: a non-holder

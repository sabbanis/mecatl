...
| `--main-retention-max-total` | `0` | store-wide main-session cap; `0` disables it. Enabling requires explicit acknowledgement. |
| `--acknowledge-main-retention` | `false` | acknowledge destructive main cleanup after reviewing the logged `retention/v1` planner summary; YAML twin: `retention.acknowledge_main_deletion`. |
| `--child-gc-interval` | `1h` | repeat cadence after startup; `0` disables repeat cadence while preserving the compatibility startup sweep. YAML twin: `retention.sweep_cadence`. |

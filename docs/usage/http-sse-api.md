...
| `POST /v1/sessions/{id}/fork` | `{"title": "..."}` (optional; empty/absent inherits the source's) | `201` `{session_id}` — create a peer session from `{id}`'s conversation history snapshot (ADR 0065); same provider/model only; `412` if `{id}` is running/awaiting |

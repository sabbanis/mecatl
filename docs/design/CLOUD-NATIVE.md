...
| 11 | Per-session engine registry (`Service.sessionEngines`) | `server.Service` | session | evicted + closed at `CloseSession` and shutdown | lost; rehydrated via `Service.rehydrateSession` for the no-fs profile, a persisted selector, OR an empty workspace (`needsRehydration`), AND — issue #66 engine-window fix — for an otherwise-DEFAULT session whose LIVE context window strictly exceeds the build-time baked window (`Service.defaultSessionNeedsLiveWindow`, so a live-only default model compacts at the live window, not the 128k floor); decision = derive (nothing new persisted — the rebuilt window is re-derived live-first via the same `Config.ResolveContextWindow` the echo overlay reads) | `internal/adapter/server/service.go` (`sessionEngines`) |
...
| | _(traceability)_ `reg.meta` (the refreshed `liveMetaStore`) now ALSO feeds the DEFAULT-session `resolved_model` echo via the injected `server.Config.ResolveContextWindow` closure (issue #66): the default-path context-window scalar is re-resolved live-first at call time, so a live-only model whose curated-catalog floor is 0 echoes the live window post-swap. No new resource — the closure captures the same atomic-pointer store this row owns. | | | | | `internal/app/build.go` (`ResolveContextWindow`); `internal/adapter/server/service.go` (`ResolvedModel`) |
...
| 2 | Provider/model selector | **SHIPPED (Phase 1)**: persisted as the opaque `ProviderID`/`ModelID` label pair on the aggregate (`session.Session`) + snapshot (`sessnap.Snapshot`); `Service.rehydrateSession` re-derives the SAME engine via the factory from the persisted pair (no longer the default-provider floor) | rebuilt on the SAME provider+model via the factory; a default session (empty pair) rides the shared engine UNLESS its live context window exceeds the build-time baked window (issue #66, `Service.defaultSessionNeedsLiveWindow`), in which case it ALSO rehydrates into a per-session engine carrying the live-first window (the context-window SCALAR is re-derived, never persisted) | persist-in-snapshot (selector); rehydration re-derives the engine via the factory; the engine COMPACTION window is derived live-first at rehydration (decision = derive) | 1 (SHIPPED); engine-window trigger #66 |

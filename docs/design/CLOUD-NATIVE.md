...
| | _(traceability)_ `reg.meta` (the refreshed `liveMetaStore`) ALSO feeds the `resolved_model` echo for BOTH branches via the injected `server.Config.ResolveContextWindow` closure (issue #66, promoted by the resolve-at-use unification): it is the SAME `reg.windowResolver` the engine reads via `Deps.ContextWindow`, so the echo and the running engine resolve the window from one source at call time (a live-only model whose curated-catalog floor is 0 echoes the live window post-swap; the operator `--context-window-override` wins for both). No new resource — the closure captures the same atomic-pointer store this row owns. | | | | | `internal/app/livemeta.go` (`windowResolver`); `internal/app/build.go` (`ResolveContextWindow`); `internal/adapter/server/service.go` (`ResolvedModel`) |

...
| Model-only session profile | ⚠️ implementation candidate — exact no-FS placement, construction-time empty model catalog, one-shot lifecycle, attachment/auxiliary-call rejection, and positive resource envelope satisfy the [approved acceptance plan](../acceptance/model-only-one-shot-profile.md), but remain unshipped until implementation review and merge | [0350](../adr/0350-model-only-profile-and-compaction-off.md) · [0351](../adr/0351-bounded-one-shot-model-only-runs.md) | [overview](../architecture.md) · [fork patch inventory](./QUALIFICATION-FORK-PATCHES.md) |

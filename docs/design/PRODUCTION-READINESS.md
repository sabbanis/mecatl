...
| Studio (web client) | ✅ Atrium workspace shipped daemon-only: five surfaces (Chats · Scheduled · Skills · Memory · Settings) on the hardened proxy tier, managed/external modes, hermetic server-tier + protocol + e2e suites · ⛔ skill & agent-definition authoring (controller-mediated) · ⛔ learned-skills / learning-proposals UI · ⛔ plan-mode approval flow · ⛔ live re-attach to running sessions (needs an HTTP/SSE mirror of gRPC `StreamSessionLive`) · ⛔ generated TS proto bindings | [0233](../adr/0233-studio-atrium-module.md) · [0234](../adr/0234-studio-server-backed-chats.md) | [overview](../architecture.md) |

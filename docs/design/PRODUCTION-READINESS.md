...
| Direct MCP OAuth dynamic client registration | 🔨 Core trusted-profile validation, durable CAS registration, generation-bound no-refresh grant, explicit reset/retry recovery, restart reuse, expiry-to-login-required, protocol/security fixtures, seven named acceptance proofs, and cloud-native resource re-audit complete · ⛔ repeat panel review, implementation PR/merge, and live local-mecatui qualification remain · ⛔ complete resource-bound public-client refresh ([issue #1355](https://github.com/stacklok/mecatl/issues/1355)) | [0325](../adr/0325-direct-mcp-dcr.md) | [credential store](../architecture.md#internal-credential-store) |
...
| Cloud-native learning | ✅ ADR 0259 landed: deterministic durable attempts are the workflow authority; bounded repository discovery and claim renewal drive exact evidence-only execution; attempt and automatic-reservation expiry use backend-authoritative time; orphan reservations are durably discovered and reconciled; setup/evidence-not-ready recovery uses persisted exponential backoff and terminates as `retry_exhausted` after three failed claims; distributed proposal/skill convergence, replica-safe learned-skill hydration, content-free attempt APIs, and globally bounded automatic accounting ship when the durable repositories/ledger are wired · ⚠️ raw remote learning drivers are trusted single-tenant infrastructure only and fail closed with application ownership enforcement · ⚠️ independent stores provide no cross-store transaction and may admit a compatible late downstream commit that never proves attempt success · ⛔ multi-tenant remote learning pending ADR 0213 workload authentication/private owner registry/separate maintenance RPCs · ⛔ attempt watch/change feed (ADR 0250 session watch is not a substitute) | [0259](../adr/0259-cloud-native-learning.md) · [0027](../adr/0027-cloud-native.md) | [evidence-backed reflection](../architecture.md#evidence-backed-reflection) · [memory](../architecture/memory.md) |
| Compaction (heuristic + cascade shipped; explicit off candidate) | ⚠️ `off` satisfies its approved source contract in the implementation candidate; it remains unshipped until implementation review and merge | [COMPACTION.md](../adr/0012-compaction.md) · [0350](../adr/0350-model-only-profile-and-compaction-off.md) | [context & compaction](../architecture/context-and-compaction.md) |
| Model-only session profile | ✅ fork baseline `a607746a59b1ad61ac2331db7827b31b0a03919d` merged on the pinned fork branch with exact no-FS placement, an empty catalog, one-shot lifecycle, auxiliary-path rejection, and positive resource ceilings · ⛔ no immutable fork artifact or approved non-mock operator configuration yet · ⛔ not an upstream or general Mecatl release | [0350](../adr/0350-model-only-profile-and-compaction-off.md) · [0351](../adr/0351-bounded-one-shot-model-only-runs.md) | [acceptance plan](../acceptance/model-only-one-shot-profile.md) · [fork patch inventory](./QUALIFICATION-FORK-PATCHES.md) |
| Fork-only qualification release | 📝 proposed — closed `v0.0.39-i2i.N` source authority, `mecated`-only native archives, disabled product metrics, manifest, SBOMs, checksums, fork-owned Sigstore bundles, and SLSA provenance · ⛔ implementation review/merge and first tagged release remain | [0352](../adr/0352-controlled-fork-qualification-release.md) | [acceptance plan](../acceptance/fork-only-qualification-release.md) |
...
| Guardrails (LLM-backed tool-content inspection) | ✅ shipped | [0021](../adr/0021-guardrails.md) · [0241](../adr/0241-governance-fence-ownership.md) (canonical fence ownership) · [0046](../adr/0046-guardrails-slot-enable.md) · [0049](../adr/0049-guardrails-remove-maxchecks.md) · [0050](../adr/0050-guardrails-remove-maxcontentbytes.md) · [0051](../adr/0051-guardrails-advisory-tui-visibility.md) · [0052](../adr/0052-guardrails-checker-down-toggle.md) · [0053](../adr/0053-guardrails-default-block.md) | [hooks & guardrails](../architecture/hooks-and-guardrails.md) |
...
| Perf observability (live admin/MCP) | ✅ Phases 1+2; embedded mecatui defaults to a private per-instance admin socket, with loopback TCP only when explicit or required by streaming-HTTP MCP · 🟦 Phase 3 (fleet/Pyroscope, optional) | [perf-observability.md](../adr/0018-perf-observability.md) · [0254](../adr/0254-session-debugger-admin-transport.md) | [observability](../architecture/observability.md) |
| Dedicated stored-session debugger | ✅ target-bound durable no-fs debug session; one TARGET grammar for authoritative exact IDs or collision-aware displayed handles; bounded fenced status/transcript/activity/performance/network evidence; related Subagent/Parallel/Team/schedule handles and pruning tombstones; delegation, compaction history, request manifests, latest-vs-lifetime counters; selected streaming-HTTP MCP reporting with a mandatory fresh approval per mutation; restart-safe rehydration, hierarchical first prompt, privacy disclosure, and DEBUG identity · ⛔ live target following, raw audit/tool records, raw logs/pprof, packet capture, support bundles | [0254](../adr/0254-session-debugger-admin-transport.md) · [0255](../adr/0255-sanitized-network-attempt-evidence.md) · [0256](../adr/0256-session-debugger-evidence-and-reporting.md) · [0285](../adr/0285-predictable-mecatui-session-handles.md) | [overview](../architecture.md) · [TUI](../tui.md#debug-a-stored-session) · [observability](../architecture/observability.md) |
...
| **OIDC caller identity** | ✅ attribution via shipped `toolhive-core/authn` v0.0.39; ✅ per-caller authorization/isolation (#368); ✅ default 1h JWKS-staleness bound (503 after an unavailable refresh); ⛔ per-token revocation | `--oidc-max-jwks-staleness=0` deliberately restores unbounded cached-key availability; JWKS cache is process-local and reconstructible, never persisted. |
...
| Explicit compaction-off | ⚠️ local candidate | `--compaction=off` removes the automatic context-window trigger and makes manual compaction fail with `ErrCompactionDisabled`; proposed by ADR 0350 and not shipped before plan/implementation review |
...
|---|---|---|
| TypeScript SDK Deno integration | 🟦 Deferred beyond v0.1.0 | Native `Deno.Command` local ownership and shared ConnectRPC gRPC qualification are under review in draft [PR #1423](https://github.com/stacklok/mecatl/pull/1423), explicitly excluded from v0.1.0. No later release version is assigned. See the [acceptance contract](../acceptance/sdk-typescript-deno.md). |

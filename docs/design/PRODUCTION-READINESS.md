...
|---|---|---|---|
| Native Kubernetes execution provider | 🔨 Human-authorized draft development in existing [PR #1614](https://github.com/stacklok/mecatl/pull/1614), not shipped or approved. N2 candidate retains workload isolation, authority/capacity ledgers, and configuration through uninstall; ownership-checked lookup preserves data for same-release/namespace adoption and rejects incompatible profiles or missing history. Offline real-Helm rendering coverage and a production Kind install/upgrade/uninstall/reinstall test are authored. ⛔ Final deterministic CI, enforcing-CNI lifecycle execution, native real-provider coding smoke, API/schema/security reviews, and final acceptance evidence remain pending. Generic live Kubernetes proof is not native execution qualification. | [0349](../adr/0349-native-kubernetes-execution.md) · [acceptance contract](../acceptance/native-kubernetes-execution.md) | [native execution mechanics](./IMPLEMENTATION-NOTES.md#native-kubernetes-execution-provider) |

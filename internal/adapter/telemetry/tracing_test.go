...
func TestTracingRunSpan(t *testing.T) {
	...
	spans := exp.GetSpans()
	run, ok := spanByName(spans, "ozz.run")
	if !ok {
		t.Fatalf("no ozz.run span; got %d spans", len(spans))
	}
	if run.Status.Code != codes.Ok {
		t.Errorf("run status = %v, want Ok", run.Status.Code)
	}
	if v, ok := attrInt(run, "ozz.tokens.input"); !ok || v != 100 {
		t.Errorf("ozz.tokens.input = %v (ok=%v), want 100", v, ok)
	}
	if v, ok := attrInt(run, "ozz.tokens.cache_read"); !ok || v != 20 {
		t.Errorf("ozz.tokens.cache_read = %v (ok=%v), want 20", v, ok)
	}
	var hasStop bool
	for _, kv := range run.Attributes {
		if kv.Key == attribute.Key("ozz.run.stop") && kv.Value.AsString() == "end_turn" {
			hasStop = true
		}
	}
	if !hasStop {
		t.Errorf("run span missing ozz.run.stop=end_turn")
	}
}
...
func TestTracingToolChildSpan(t *testing.T) {
	...
	spans := exp.GetSpans()
	tool, ok := spanByName(spans, "ozz.tool")
	if !ok {
		t.Fatalf("no ozz.tool span; got %d spans", len(spans))
	}
	run, ok := spanByName(spans, "ozz.run")
	if !ok {
		t.Fatal("no ozz.run span")
	}
	turn, ok := spanByName(spans, "ozz.turn")
	if !ok {
		t.Fatal("no ozz.turn span")
	}
	...
	for _, kv := range tool.Attributes {
		if kv.Key == attribute.Key("ozz.tool.name") && kv.Value.AsString() == "bash" {
			nameOK = true
		}
	}
	if !nameOK {
		t.Errorf("tool span missing ozz.tool.name=bash")
	}
}
...
func TestTracingErrorStopReason(t *testing.T) {
	...
	run, ok := spanByName(exp.GetSpans(), "ozz.run")
	if !ok {
		t.Fatal("no ozz.run span")
	}

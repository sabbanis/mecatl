...
redis.call('DEL', KEYS[1], KEYS[4], KEYS[5], KEYS[7], KEYS[8])
...
func deleteSessionAndMetadata(ctx context.Context, client redis.UniversalClient, id session.SessionID) error {
	return deleteMetadataScript.Run(ctx, client,
		[]string{sessionKey(id), metadataGlobalIndexKey, metadataGenerationKey, toolsKey(id), eventsKey(id), metadataRebuildGenerationKey, eventsGenerationKey(id), ledgerKey(id)},
...
func deleteSessionIfMetadataUnchanged(ctx context.Context, client redis.UniversalClient, expected port.SessionDiscoveryMeta) (bool, error) {
	...
	result, err := conditionalDeleteMetadataScript.Run(ctx, client,
		[]string{sessionKey(expected.ID), metadataGlobalIndexKey, metadataGenerationKey, toolsKey(expected.ID), eventsKey(expected.ID), metadataRebuildGenerationKey, eventsGenerationKey(expected.ID), ledgerKey(expected.ID)},

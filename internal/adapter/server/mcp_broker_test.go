...
func TestMCPBrokerCarryoverDoesNotWidenAttenuatedAuthority(t *testing.T) {
	...
	defer runtime.Close()
	store := memstore.New()
	source := session.New("attenuated-source", session.ModeDefault, "/workspace", session.Limits{}, time.Now())
	...
	carried, err := service.CreateSessionWithProfile(t.Context(), "/workspace", session.ModeDefault, session.Limits{}, ProviderSelector{}, ProfileDefault, WithSourceSession(source.ID))
	if err != nil {
...
func TestMCPBrokerCanonicalAttachPersistReattachAndLocalClose(t *testing.T) {
	...
	created, err := service.CreateSession(context.Background(), "/workspace", session.ModeDefault, session.Limits{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMCPBrokerExplicitDeleteFailureKeepsHostRetryable(t *testing.T) {
	...
	defer runtime.Close()
	created, err := service.CreateSession(t.Context(), "/workspace", session.ModeDefault, session.Limits{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMCPBrokerAttachRollbackSerializesAgainstReattachCommit(t *testing.T) {
	...
	go func() {
		_, createErr := service.CreateSession(context.Background(), "/workspace", session.ModeDefault, session.Limits{})
...
func TestMCPBrokerServiceShutdownBoundsAttachmentDrain(t *testing.T) {
	...
	if _, err := service.CreateSession(context.Background(), "/workspace", session.ModeDefault, session.Limits{}); err != nil {
		t.Fatal(err)
	}
	callDone := make(chan error, 1)
	go func() {

...
type Config struct {
	...
	DeploymentID string
	// DetachedRuns is the operator-tier gate for detached runs (ADR 0322,
	...
	// once (ADR 0322 decision 6: the concurrency gate, mirroring the engine's
	...
	// the "forgot to come back" bound (ADR 0322 decision 6). A detached run has
	...
	// DetachedRunsRefused is the POSTURE-derived detached-run refusal (ADR 0322
...
// applyDetachedRunDefaults fills the zero-value detached-run defaults (ADR 0322
...
// when Config.MaxDetachedRuns is zero (ADR 0322, the task brief's "~4"). Four
...
// run when Config.DetachedRunDeadline is zero (ADR 0322 decision 6: the
...
type Service struct {
	...
	// detachedGate is the server-wide detached-run counting semaphore (ADR 0322
...
// (StartDetachedRunContent's drain goroutine, ADR 0322): the drain goroutine is
...
func NewServiceContext(ctx context.Context, cfg Config) (*Service, error) {
	...
	// Size the detached-run concurrency gate (ADR 0322 decision 6). A negative
...
// DetachedRuns reports the operator-tier detached-runs gate (ADR 0322,
...
// drained server-side.
//
// Security guardrails (ADR 0322 decision 6 — Scenario 5 hardening):
...
// concurrency gate (ADR 0322 decision 6), failing FAST when the gate is full. It

...
//nolint:gocyclo // Session creation is one ordered admission transaction: reserve identity before attachments and persistence.
func (s *Service) createSession(ctx context.Context, workspace string, mode session.PermissionMode, limits session.Limits, sel ProviderSelector, specs []mcp.ServerConfig, profile SessionProfile, opts createSessionOpts) (*session.Session, error) {
	...
	needPerSession := s.cfg.MCPBroker != nil || s.sessionNeedsPerFactory(sel, specs, profile, workspace) ||
...
func (s *Service) startRunContent(ctx context.Context, id session.SessionID, text string, parts []session.Content, purpose runPurpose, canPresentAuthorization bool) (*agent.Run, error) {
	if text == "" && len(parts) == 0 {
	...
	runID := newRunID()
	sess.BeginRun(runID)
	run := engine.Run(ctx, sess, env, agent.RunRequest{Text: text, Parts: parts, RunID: runID, CanPresentAuthorization: canPresentAuthorization})
	s.register(id, run, sess)
	return run, nil
}
...
func (s *Service) needsRehydration(sess *session.Session) bool {
	return s.cfg.MCPBroker != nil || s.cfg.LearnedSkills != nil && sess.Owner != nil && sess.Owner.Issuer != "" && sess.Owner.Subject != "" ||
...
func (s *Service) buildAndRegisterSessionEngine(ctx context.Context, sess *session.Session, sel ProviderSelector, profile SessionProfile, mode session.PermissionMode, replace bool) (*sessionEngine, error) {
	...
	if sess.Kind == session.SessionKindDebug {
		...
		res, err = s.callSessionEngine(ctx, sel, nil, profile, sess.Workspace, mode, brokerTools(broker))
	}
	if err != nil {

...
var serviceAccessTable = map[string]ClassificationEntry{
	...
	"CreateProject":             {KindCallerOwned, "binds the verified context principal as Project owner and resolves only a registered working source"},
	"GetProject":                {KindCallerOwned, "loadOwnedProject conceals foreign and missing Project identifiers identically"},
	"ListProjects":              {KindCallerOwned, "passes the verified principal to the store before page formation and count"},
	"ReplaceProject":            {KindCallerOwned, "loadOwnedProject authorizes before source resolution and revision-CAS replacement"},
	"DeleteProject":             {KindCallerOwned, "loadOwnedProject authorizes before revision-CAS deletion"},
	"CreateSessionFromProject":  {KindCallerOwned, "loadOwnedProject authorizes before source resolution and before the captured Session is persisted"},
	...
	"ListProjectWorkingSources": {KindSharedInfrastructure, "the immutable operator-registered working-source inventory, identical for every admitted caller"},

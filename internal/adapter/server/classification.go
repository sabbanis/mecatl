...
var serviceAccessTable = map[string]ClassificationEntry{
	...
	"ReattachPlacementForBinding": {KindCallerOwned, "passes the exact persisted ref and session binding with the verified context principal to the provider"},
	...
	"SteerRun":                     {KindCallerOwned, "authorizes via GetSession before steering the exact addressed live run without promotion"},
	"CancelSteer":                  {KindCallerOwned, "authorizes via GetSession before reaching into the live run's steer inbox"},
	"CancelRunSteer":               {KindCallerOwned, "authorizes via GetSession before retracting steering from the exact addressed live run"},

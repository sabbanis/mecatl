...
func profileRecordingFactory(reply string, got *atomic.Value, calls *atomic.Int32) server.SessionEngineFactory {
	return func(_ context.Context, _ server.ProviderSelector, _ []mcp.ServerConfig, profile server.SessionProfile) (server.SessionEngineResult, error) {
		calls.Add(1)
		got.Store(profile)
...
func TestNoFSSessionUsesWorkspaceOverride(t *testing.T) {
	...
	svc, err := server.NewService(server.Config{
		...
		SessionEngine: func(_ context.Context, _ server.ProviderSelector, _ []mcp.ServerConfig, _ server.SessionProfile) (server.SessionEngineResult, error) {

...
func newLifecycleFixtureWithTurns(t *testing.T, status session.AuthorizationStatus, attachErr error, now func() time.Time, timer AuthorizationTimerFactory, turns ...mockllm.Turn) lifecycleFixture {
	...
	cfg := Config{
		Engine: shared, Store: store, Workspaces: func(root string) tool.Workspace { return memfs.NewWorkspace(root) },
	...
	sess := session.New("authorization-session", session.ModeDefault, "/repo", session.Limits{}, now())
...
func TestMCPAuthorizationContinuationReplacesJustParkedRun(t *testing.T) {
	...
	parkingSession := session.New("parking-session", session.ModeDefault, "/repo", session.Limits{}, time.Now())
	parkingEnv, err := tool.NewEnvironment(session.EnvironmentRef{Kind: session.EnvKindMem, ID: "/repo"}, memfs.NewWorkspace("/repo"), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	f.svc.mu.Lock()
	f.svc.runs[id] = &runState{run: parkingRun, sess: parkingSession}
	f.svc.mu.Unlock()
...
func TestMCPAuthorizationTerminalFallbackIsFullyReconstructable(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(string(tc.brokerStatus), func(t *testing.T) {
			...
			loaded.EnvironmentRef = session.EnvironmentRef{Kind: session.EnvironmentKind("lost-remote"), ID: "runtime"}
			...
			if _, err := eventsource.Fold(eventsource.SessionMeta{
				ID: loaded.ID, Mode: loaded.Mode, Limits: loaded.Limits, Workspace: loaded.Workspace, CreatedAt: loaded.CreatedAt,
...
func TestMCPAuthorizationPreparedRegistrationCancellationRestoresClaim(t *testing.T) {
	...
	prepared, err := engine.PrepareAuthorizationContinuation(memory.WithWorkspace(ctx, loaded.Workspace), loaded, env, claimed, resolution)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMCPAuthorizationPreparedRegistrationFailureNeverExecutes(t *testing.T) {
	...
	blockerSession := session.New("blocker", session.ModeDefault, "/repo", session.Limits{}, time.Now())
	blockerEnv, err := tool.NewEnvironment(session.EnvironmentRef{Kind: session.EnvKindMem, ID: "/repo"}, memfs.NewWorkspace("/repo"), nil)
	if err != nil {
		t.Fatal(err)
	}
	blocker := f.svc.cfg.Engine.Run(t.Context(), blockerSession, blockerEnv, agent.RunRequest{Text: "block"})
	f.svc.mu.Lock()
	f.svc.runs[loaded.ID] = &runState{run: blocker, sess: blockerSession}
	f.svc.mu.Unlock()
	control := MCPAuthorizationControl{SessionID: loaded.ID, AuthorizationID: f.pending.Authorization.ID}
	result, err := f.svc.RecheckMCPAuthorization(t.Context(), loaded.ID, control)

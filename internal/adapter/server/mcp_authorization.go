...
func (s *Service) MCPAuthorizationPresentation(ctx context.Context, id session.SessionID, control MCPAuthorizationControl) (string, error) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "DEBUGTRACE MCPAuthorizationPresentation PresentAuthorization FAILED err=%v\n", err)
		return "", err
	}
	fmt.Fprintf(os.Stderr, "DEBUGTRACE MCPAuthorizationPresentation RETURNING url=%q\n", url)
	return url, nil
}
...
func (s *Service) continueGrantedAuthorizationLocked(ctx context.Context, sess *session.Session) (MCPAuthorizationResult, error) {
	...
	prepared, err := engine.PrepareAuthorizationContinuation(memory.WithWorkspace(ctx, sess.Workspace), sess, env, claimed, resolution)
	if err != nil {
...
func (s *Service) resolveAuthorizationLocked(ctx context.Context, sess *session.Session, pending session.PendingAuthorization, status session.AuthorizationStatus) (MCPAuthorizationResult, error) {
	...
	prepared, err := engine.PrepareAfterAuthorization(memory.WithWorkspace(ctx, sess.Workspace), sess, env, pending.Authorization, pending.Call.ID, results, resolution)
	if err != nil {
...
func (s *Service) settleAuthorizationLocked(ctx context.Context, id session.SessionID) error {
	...
	if err := s.cfg.Store.Save(context.WithoutCancel(ctx), sess); err != nil {

...
func TestNotificationStormLogsOncePerRefreshCycle(t *testing.T) {
	...
	// The WARN must have fired exactly ONCE (the false→true transition), not
	// 5× (once per AddTool notification). A count > 1 is a notification-storm
	// log-volume DoS the dedup is meant to prevent.
	if got := diag.count("mcp server list changed"); got != 1 {
		t.Errorf("notification-storm WARN count = %d, want 1 (deduped to the false→true transition)", got)
	}
}

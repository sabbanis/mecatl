...
type Config struct {
	...
	// sub-keys = error, like permissions:/guardrails:) so a typo cannot silently
	// disable a schedule. It is honoured ONLY from the user-global + CLI tiers; a
	// project-tier file's schedules: block is IGNORED with a WARN (a project repo
	// registering schedules is an operator decision — the same operator-tier-only
	// discipline as guardrails/posture). A nil Schedules means the key was absent.
	Schedules *SchedulesSection `yaml:"schedules"`
}
...
// SchedulesSection is the operator-tier `schedules:` YAML subtree (issue #233,
// Phase 2b): a LIST of schedule declarations (the `schedules:` key is a YAML
// sequence, not a mapping). It is parsed STRICTLY at the per-element grain: each
// list element is decoded as a ScheduleDecl whose own UnmarshalYAML rejects
// unknown keys, so a typo'd key inside one declaration cannot silently disable a
// schedule. The top-level Config stays lenient; this subtree is strict.
//
// The YAML shape is:
//
//	schedules:
...
//	    prompt: "..."
//
// (a bare sequence under `schedules:` — no `items:` wrapper).
type SchedulesSection struct {
	// Items is the list of schedule declarations. It has no yaml tag because the
	// SchedulesSection itself is the sequence node (UnmarshalYAML decodes the
	// sequence directly into Items), not a mapping with an `items:` key.
	Items []ScheduleDecl
}

// UnmarshalYAML decodes the schedules: sequence STRICTLY at the per-element grain
// (issue #233): the `schedules:` key is a YAML sequence, and each element is
// decoded as a ScheduleDecl whose own UnmarshalYAML rejects unknown keys. A
// non-sequence node (e.g. a stray mapping) is a parse error. A null/absent node
// (a bare `schedules:` line) decodes to an empty list.
func (s *SchedulesSection) UnmarshalYAML(node *yaml.Node) error {
	if node == nil || node.Tag == "!!null" {
		return nil
	}
	if node.Kind != yaml.SequenceNode {
		return fmt.Errorf("schedules: expected a sequence (line %d)", node.Line)
	}
	s.Items = make([]ScheduleDecl, 0, len(node.Content))
...
// ScheduleDecl is one operator-tier schedule declaration as parsed from YAML. It is
// the YAML-friendly mirror of port.ScheduleSpec: cron/oneShot are bare strings (the
// composition layer parses the cron expression via cronparse and the one-shot instant
// via time.Parse), provider/model are opaque selector strings, and the limits are
// the per-fire turn/tool-call budgets. Parsed strictly (unknown keys error).
//
// Exactly one of Cron / OneShot is set (the composition mapper enforces the XOR via
// port.TriggerSpec.Validate). A declaration with neither is invalid.
type ScheduleDecl struct {
	// Name is the schedule's unique key (the ScheduleStore upserts by Name).
	Name string `yaml:"name"`
	// Cron is the cron expression (5-field or @-macro). Mutually exclusive with
	// OneShot. Empty unless this is a cron trigger.
	Cron string `yaml:"cron"`
	// OneShot is an RFC3339 wall-clock instant the schedule fires once at. Mutually
	// exclusive with Cron. Empty unless this is a one-shot trigger. It MUST be in
	// the future at Build time (the create-seam validates this).
	OneShot string `yaml:"oneShot"`
	// Timezone is the IANA timezone name the cron expression fires in. Empty means
	// UTC. Ignored for a one-shot trigger (an absolute instant is tz-aware).
	Timezone string `yaml:"timezone"`
	// Prompt is the free-text user prompt the fire runs with.
	Prompt string `yaml:"prompt"`
	// Provider is the opaque provider id ("" = deployment default).
	Provider string `yaml:"provider"`
	// Model is the opaque model id ("" = deployment default for the provider).
	Model string `yaml:"model"`
	// Profile is the session tool-surface profile ("" default, "no-fs" file-less).
	Profile string `yaml:"profile"`
	// Workspace is the session cwd ("" = deployment default).
	Workspace string `yaml:"workspace"`
	// Mode is the session permission posture ("default"/"plan"/"acceptEdits"). A
	// non-mutating schedule (Mutating=false) MUST use "plan".
	Mode string `yaml:"mode"`
	// Mutating is the explicit write opt-in. false (the default) = read-leaning.
	Mutating bool `yaml:"mutating"`
	// MaxFires bounds the TOTAL number of fires for a cron schedule (0 = forever).
	// Ignored for a one-shot (fires once by definition).
	MaxFires int `yaml:"maxFires"`
	// MaxTurns caps the number of model calls per fire (0 = disabled).
...
func (d *ScheduleDecl) strictFields() map[string]any {
	return map[string]any{
		"name":         &d.Name,
		"cron":         &d.Cron,
		"oneShot":      &d.OneShot,
...
// UnmarshalYAML decodes a schedules list-element mapping STRICTLY: an unknown key
// inside a schedule declaration is a parse error (same rationale as the parent
// subtree). The `where` is "schedules[]" so the operator sees the path.
func (d *ScheduleDecl) UnmarshalYAML(node *yaml.Node) error {
	return decodeStrictMapping(node, "schedules[]", d.strictFields())
}

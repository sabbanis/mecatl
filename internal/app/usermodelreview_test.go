...
func minimalReviewer(t *testing.T, store port.SessionStore) *agent.UserModelReviewer {
	t.Helper()
	eng := newChildEngine(mockllm.New(mockllm.TextTurn("x")), tool.NewCatalog(), "test-model", promptConfig(Config{}))
...
// — no Read/Edit/Write/Bash/Task/Fork. So the reviewer can WRITE the user model but
...
func TestUserModelReviewEngineCatalogIsMinimal(t *testing.T) {
	...
	for _, banned := range []string{"Read", "Edit", "Write", "Bash", "Task", "Fork", memory.RecallUserToolName, memory.SearchUserModelToolName} {
...
func reviewEngineToolNames(provider port.LLMProvider, store *memory.Store) []string {
	...
	_ = newChildEngine(provider, cat, "test-model", promptConfig(Config{}))
	var names []string

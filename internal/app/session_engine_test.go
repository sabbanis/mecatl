...
func TestSessionEngineFactoryBuildsUsableEngine(t *testing.T) {
	...
	factory := sessionEngineFactory(cfg, reg, provider, store, policy, hooks, nil, nil, prompt.RootAssembler{}, nil)
...
func twoProviderFactory(t *testing.T) (server.SessionEngineFactory, *providerRegistry) {
	...
	store := memstore.New()
	policy := permpolicy.NewPolicy(defaultRules(), nil)
	factory := sessionEngineFactory(cfg, reg, oa, store, policy, hookexec.New(nil), nil, nil, prompt.RootAssembler{}, nil)
	return factory, reg
}

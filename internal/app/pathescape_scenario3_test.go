...
func TestPathEscapePosture_Scenario3_EditLedgerOutOfRoot(t *testing.T) {
	...
	env, err := tool.NewEnvironment(session.EnvironmentRef{Kind: session.EnvKindLocal, ID: f.workspace}, gate, testReadLedger(), nil)
	if err != nil {
...
func TestPathEscapePosture_Scenario3_WriteEscapeMutateSerial(t *testing.T) {
	...
	env, err := tool.NewEnvironment(session.EnvironmentRef{Kind: session.EnvKindLocal, ID: f.workspace},
		newSerialProbeWorkspace(t, f.workspace, &inflight, &maxSeen, 50*time.Millisecond), testReadLedger(), nil)
	if err != nil {
		t.Fatalf("NewEnvironment: %v", err)
	}
	built.Service.SetSessionEnvironment(sess.ID, env)

...
func TestUntrustedReadOnlyMemberHasNoBash(t *testing.T) {
	cfg := untrustedTeamCfg(t)
	prov := bashThenEdit()
	factory, _, _, _ := buildTeamWiring(context.Background(), cfg, regForTest(prov, providerMock, cfg.Model), prov, providerMock, cfg.Model, nil, agents.NewRegistry(nil), nil, nil)
...
func TestUntrustedMutatingDefMemberKeepsBash(t *testing.T) {
	...
	factory, _, _, _ := buildTeamWiring(context.Background(), cfg, regForTest(prov, providerMock, cfg.Model), prov, providerMock, cfg.Model, nil, regOf(def), nil, nil)

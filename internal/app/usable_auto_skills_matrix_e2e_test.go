		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for time.Now().Before(deadline) {
				...
				if err != nil {
					...
					t.Fatalf("ListLearnedSkills failed: category=%s provider_calls=%d proposal_count=%d proposal_error_category=%s diagnostic_count=%d", learnedSkillDiagnosticCategory(err), provider.Calls(), len(proposals.GetProposals()), learnedSkillDiagnosticCategory(proposalErr), len(diag.capturedStrings()))
				}
				if tc.wantState == "" {
			...
			if tc.wantState == "" {
				...
				if len(listed.GetSkills()) != 1 {
					proposals, proposalErr := built.Service.ListLearningProposals(ctx, "", "", 50, workspace)
					t.Fatalf("learned_skill_count=%d provider_calls=%d proposal_count=%d proposal_error_category=%s diagnostic_count=%d", len(listed.GetSkills()), provider.Calls(), len(proposals.GetProposals()), learnedSkillDiagnosticCategory(proposalErr), len(diag.capturedStrings()))
				}
				got := listed.GetSkills()[0]
				if learning.SkillState(got.GetState()) != tc.wantState || len(got.GetReceipts()) == 0 || got.GetReceipts()[len(got.GetReceipts())-1].GetOperation() != tc.wantOperation {
					lastOperation := "none"

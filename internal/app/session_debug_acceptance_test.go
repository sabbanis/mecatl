...
func TestSessionDebuggerCrossBoundaryAcceptance(t *testing.T) {
	...
	if !strings.Contains(scoped["child"], "retained child finding") || !strings.Contains(scoped["history-page"], `"view":"transcript"`) || !strings.Contains(scoped["probe"], "unsupported, malformed, or stale") {

		...
		t.Run(map[bool]string{false: "deleted", true: "recreated"}[recreate], func(t *testing.T) {
			...
			provider := mockllm.NewWith([]mockllm.Option{
				mockllm.WithRequestObserver(func(req port.LLMRequest) {
					requests.Add(1)
					if !slices.ContainsFunc(req.Tools, func(spec tool.ToolSpec) bool { return spec.Name == "Bash" }) {
						t.Error("main or direct-write child request lost Bash")
					}
				}),
			},
				mockllm.ToolCallTurn(session.NewToolCall("before", "Bash", []byte(`{"command":"printf before-recovery"}`))),
				mockllm.TextTurn("first run complete"),
				mockllm.ToolCallTurn(session.NewToolCall("after", "Bash", []byte(`{"command":"printf after-recovery"}`))),
				mockllm.ToolCallTurn(session.NewToolCall("delegate", "Subagent", []byte(`{"prompt":"write the proof using Bash","mode":"read-write"}`))),
				mockllm.ToolCallTurn(session.NewToolCall("child-bash", "Bash", []byte(`{"command":"printf child-recovered > recovered-child.txt"}`))),
			...
			if err != nil || string(data) != "child-recovered" {
				t.Fatalf("direct-write child did not execute Bash in parent workspace: %q, %v", data, err)
			}
...
func TestManagedTempRecoveryWritableChildConstruction(t *testing.T) {
	...
	provider := mockllm.NewWith([]mockllm.Option{mockllm.WithRequestObserver(func(port.LLMRequest) {
		...
		mockllm.ToolCallTurn(session.NewToolCall("proof", "Bash", []byte(`{"command":"printf constructed > constructed-child.txt"}`))),
	...
	if err != nil || string(data) != "constructed" {
		t.Fatalf("child constructed after deletion lost Bash: %q, %v", data, err)
	}
}
...
func TestManagedTempRecoveryFailureRejectsContinuedSession(t *testing.T) {
	...
	provider := mockllm.NewWith([]mockllm.Option{
		...
		mockllm.ToolCallTurn(session.NewToolCall("must-not-start", "Subagent", []byte(`{"prompt":"requires Bash","mode":"read-write"}`))),
	)
	cfg := recoveryBuildConfig(t, provider)
	built, err := Build(context.Background(), cfg)
	if err != nil {
...
func TestManagedTempRecoveryFailureDeclinesWritableChild(t *testing.T) {
	...
	provider := mockllm.NewWith([]mockllm.Option{
		mockllm.WithRequestObserver(func(port.LLMRequest) { requests.Add(1) }),
	}, mockllm.ToolCallTurn(session.NewToolCall("retry-bash", "Bash", []byte(`{"command":"printf retried > retry-proof.txt"}`))), mockllm.TextTurn("recovered"))
	...
	for name, args := range map[string]string{
		"read-only":      `{"prompt":"requires Bash"}`,
		"writable":       `{"prompt":"requires Bash","mode":"read-write"}`,
		"writable-model": `{"prompt":"requires Bash","mode":"read-write","model":"override"}`,
	} {
	...
	if err != nil || string(data) != "retried" || requests.Load() != 2 {
		t.Fatalf("retry did not execute Bash: %q, %v, requests=%d", data, err, requests.Load())
	}
}
	...
	for _, profile := range []server.SessionProfile{server.ProfileDefault, server.ProfileNoFS} {
		t.Run(string(profile), func(t *testing.T) {
			...
			provider := mockllm.NewWith([]mockllm.Option{mockllm.WithRequestObserver(func(req port.LLMRequest) {
				requests.Add(1)
				if slices.ContainsFunc(req.Tools, func(spec tool.ToolSpec) bool { return spec.Name == "Bash" }) {
					t.Error("intentional shell-less session advertised Bash")
				}
			})}, mockllm.TextTurn("ready"), mockllm.TextTurn("still ready"))
			cfg := recoveryBuildConfig(t, provider)
			cfg.NoBash = profile == server.ProfileDefault
			built, err := Build(context.Background(), cfg)
			if err != nil {
				t.Fatal(err)
			}
...
func assertRecoveryRun(t *testing.T, svc *server.Service, id session.SessionID, output string) {
	...
	for ev := range run.Events() {
		if ev.RequestManifest != nil {
			manifests++
			if !slices.Contains(ev.RequestManifest.ToolNames, "Bash") {
				t.Error("continued request manifest omitted Bash")
			}
			var advertised bool
			for _, decision := range ev.RequestManifest.ToolDecisions {
				if decision.Name == "Bash" {
					...
					if decision.Decision == session.RequestToolMountUnavailable {
						t.Error("recovered Bash reported mount_unavailable")
					}
				}
			}
			if !advertised {
				t.Error("request manifest lacks Bash advertised decision")
			}
		}
	...
	if manifests == 0 || bashResults == 0 || !completed {
		t.Fatalf("missing successful Bash evidence: manifests=%d output=%d completed=%v", manifests, bashResults, completed)
	}
}

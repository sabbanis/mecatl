...
func twoProviderFactoryWithAgents(t *testing.T, defs *agents.Registry, taskAgent string, diag ...port.Diagnostics) (server.SessionEngineFactory, *providerRegistry) {
	...
	store := memstore.New()
	policy := permpolicy.NewPolicy([]governance.Rule{{Effect: governance.Allow}}, nil)
	factory := sessionEngineFactory(cfg, reg, oa, store, policy, hookexec.New(nil), nil, nil, nil, defs)
	return factory, reg
}
...
func TestHalfBSelectedSessionTeardownFoldsTaskClose(t *testing.T) {
	...
	store := memstore.New()
	policy := permpolicy.NewPolicy([]governance.Rule{{Effect: governance.Allow}}, nil)
	factory := sessionEngineFactory(Config{Model: "gpt-5"}, reg, oa, store, policy, hookexec.New(nil), nil, nil, nil, defs)
...
func runFactoryTaskTurn(t *testing.T, factory server.SessionEngineFactory, sel server.ProviderSelector) string {
	...
	r := res.Engine.RunContent(context.Background(), sess, memfs.NewWorkspace("/ws"), "go", nil)
	var last string
	for ev := range r.Events() {
		if ev.Type == session.EvToolResult && ev.ToolResult != nil && ev.ToolResult.Content != "" {
			last = ev.ToolResult.Content

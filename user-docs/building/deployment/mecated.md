...
| `--detached-runs` | `false` | Enable detached runs on the Converse API (ADR 0322): a `Prompt{detach:true}` first frame starts a server-owned drain goroutine that continues the run after the client disconnects, acks once, and closes the stream; the client watches via `WatchSessionEvents` and controls via a control-only Converse stream (a first frame of `cancel` / `resume_approval`). Off keeps the ATTACHED path and omits the `detached_runs` capability bit. Detached runs are refused under posture `yolo` (fail-closed) |
| `--max-detached-runs` | `4` | Server-wide cap on concurrent detached runs (ADR 0322): a counting semaphore acquired at detached-run start and released when the drain goroutine finishes. A full gate fails fast with `too_many_detached_runs` (429), listing the live detached session ids. `0` applies the sane default 4; a **negative** value disables the gate |
| `--detached-run-deadline` | `24h` | Mandatory wall-clock bound per detached run — the "forgot to come back" deadline (ADR 0322). On lapse the run is cancelled via the run-registry Cancel seam and the terminal snapshot is persisted (`stop=cancelled`, Interrupt-recoverable). The cancel is unconditional — a run parked awaiting a permission ask is cancelled too. `0` applies the sane default 24h; a **negative** value disables the deadline |

...
The flag value is `<name>=<URL>` where `name` is the identifier that becomes the namespace prefix and `URL` is the streaming-HTTP endpoint. Legacy flags and `MCP_<NAME>_TOKEN` are global-mode only. Mecated and embedded mecatui default to global; mecak8s defaults to broker, so existing mecak8s global deployments must add `mcp.mode: global` to operator settings. Broker mode accepts anonymous servers plus an ordered bundle of protected OAuth backends. Before the first prompt, the session owner uses **Connect workspace services** to complete the bundle; every protected backend must connect before authenticated discovery freezes the session catalogue. Static protected `auth.oauth.tools` declarations remain parseable comparison data but never replace the provider-reported catalogue. Restart requires fresh enrollment, and independent per-backend reconnect remains deferred. Broker mode with protected OAuth requires its configured HTTPS `mcp.broker.callback_url`; it does not use static bearer or global credential stores. Its authorization, token, vMCP, and exact callback handlers share the daemon's primary HTTPS listener—no second broker listener is created. A network-reachable broker requires verified caller identity for session ownership; ownerless broker controls are only permitted for mecated's explicit loopback single-user composition and never for mecak8s. Broker mode is currently single-process/single-replica: a non-holder fails closed rather than being transparently routed to the holder. The optional `mecatui` TUI has no `--mcp-server` flag; instead its embedded server reads operator-tier `mcp.servers` profiles from `~/.config/mecatl/settings.yaml` directly, with no flag required.

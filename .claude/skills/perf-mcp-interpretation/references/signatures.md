...
| `rss_bytes` climbs while `heap_object_bytes` stays flat (ignore `heap_allocs_total_bytes` — it is a cumulative counter and always rises) | **Off-heap growth** — invisible to Go heap/pprof. The WASM/tree-sitter leak signature (~23 MB RSS/call). The RepoMap tool that exhibited it has been removed, but the pattern holds for any off-heap consumer | Re-read `perf://runtime/memstats` over time; correlate RSS slope with off-heap activity |
